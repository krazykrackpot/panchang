...
export async function GET(req: NextRequest) {
  ...
  // Batch-fetch display names up front — one query instead of N inside
  // the loop. Gemini #691 review.
  const { data: profiles, error: profilesErr } = await supabase
    .from('user_profiles')
    .select('id, display_name')
    .in('id', userIds);
  if (profilesErr) {
    console.error('[brihaspati-abandoned-recovery] profile prefetch failed:', profilesErr.message);
    return NextResponse.json({ error: 'DB error' }, { status: 500 });
  }
  const profileMap = new Map<string, string>(
    (profiles ?? []).map((p) => [String(p.id), (p.display_name as string | null) ?? '']),
  );
  ...
  for (const userId of userIds) {
    try {
      ...
      // Safe destructure: `data` can be null on transport errors before
      // adminErr is populated. Gemini #691 review.
      ...
      // Catch-all guard — one bad row must not kill the whole run.
      // Marker stays NULL on throw, so the next cron retries the user.
      console.error('[brihaspati-abandoned-recovery] unexpected error for user', userId, ':', err);
      failed++;
    }
  }
