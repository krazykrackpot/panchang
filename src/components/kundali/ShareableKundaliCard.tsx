...
export default function ShareableKundaliCard({ kundali, locale }: Props) {
  ...
  // No &tz= — the receiving page re-resolves tz from lat/lng so a stale
  // tz cannot silently corrupt dasha computation on the shared chart.
  const shareParams = bd.name ? `?n=${encodeURIComponent(bd.name)}&d=${bd.date}&t=${bd.time}&la=${bd.lat}&lo=${bd.lng}&p=${encodeURIComponent(bd.place)}` : '';
