...
export function WeeklyClient({ rashi, locale }: WeeklyClientProps) {
  ...
  const fetchWeek = useCallback(async (id: number, slug: string) => {
    ...
    try {
      ...
      console.error('[WeeklyClient] Failed to fetch weekly horoscopes:', err);
    } finally {
      setLoading(false);
    }
  }, []);
