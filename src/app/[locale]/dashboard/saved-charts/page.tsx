...
export default function SavedChartsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(locale === 'en' || String(locale) === 'ta' ? 'Delete this chart?' : 'यह चार्ट हटाएँ?')) return;
    const supabase = getSupabase();
    if (!supabase) return;
    setDeleting(id);
  ...
  if (!user) {
    return <div className="max-w-2xl mx-auto px-4 py-20 text-center"><p className="text-text-secondary">{locale === 'en' || String(locale) === 'ta' ? 'Sign in to manage saved charts' : 'सहेजे गए चार्ट देखने के लिए साइन इन करें'}</p></div>;
  }
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="flex items-center justify-between mb-8">
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-4xl font-bold" style={hf}>
          <span className="text-gold-gradient">{locale === 'en' || String(locale) === 'ta' ? 'Saved Charts' : 'सहेजे गए चार्ट'}</span>
        </motion.h1>
        <button onClick={() => setShowForm(!showForm)}
          ...
          {showForm ? (locale === 'en' || String(locale) === 'ta' ? 'Cancel' : 'रद्द') : (locale === 'en' || String(locale) === 'ta' ? 'Add New' : 'नया जोड़ें')}
        </button>
      </div>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div className="sm:col-span-2">
                <label className="text-gold-dark text-xs uppercase tracking-wider font-bold block mb-1">{locale === 'en' || String(locale) === 'ta' ? 'Label' : 'नाम'}</label>
                <input type="text" value={label} onChange={e => setLabel(e.target.value)} placeholder={locale === 'en' || String(locale) === 'ta' ? 'e.g., My Mother' : 'उदा., मेरी माता'}
              ...
              <div>
                <label className="text-gold-dark text-xs uppercase tracking-wider font-bold block mb-1">{locale === 'en' || String(locale) === 'ta' ? 'Date of Birth' : 'जन्म तिथि'}</label>
                <input type="date" value={dob} onChange={e => setDob(e.target.value)}
                  className="w-full bg-bg-tertiary/40 border border-gold-primary/10 rounded-lg px-3 py-2 text-text-primary text-sm focus:border-gold-primary/40 outline-none" />
              </div>
              <div>
                <label className="text-gold-dark text-xs uppercase tracking-wider font-bold block mb-1">{locale === 'en' || String(locale) === 'ta' ? 'Time of Birth' : 'जन्म समय'}</label>
                <input type="time" value={tob} onChange={e => setTob(e.target.value)}
                  className="w-full bg-bg-tertiary/40 border border-gold-primary/10 rounded-lg px-3 py-2 text-text-primary text-sm focus:border-gold-primary/40 outline-none" />
              </div>
              <div className="sm:col-span-2">
                <label className="text-gold-dark text-xs uppercase tracking-wider font-bold block mb-1">{locale === 'en' || String(locale) === 'ta' ? 'Place of Birth' : 'जन्म स्थान'}</label>
      ...
      {loading ? (
        ...
        <div className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-12 text-center">
          ...
          <p className="text-text-secondary text-lg mb-2" style={bf}>{locale === 'en' || String(locale) === 'ta' ? 'No saved charts yet' : 'अभी तक कोई सहेजा गया चार्ट नहीं'}</p>
          <p className="text-text-secondary/70 text-sm mb-4">{locale === 'en' || String(locale) === 'ta' ? 'Save birth charts for family members, friends, or clients' : 'परिवार, मित्रों या ग्राहकों की जन्म कुण्डली सहेजें'}</p>
          <button onClick={() => setShowForm(true)} className="px-6 py-2 rounded-lg bg-gold-primary/20 border border-gold-primary/30 text-gold-light text-sm font-bold">
            {locale === 'en' || String(locale) === 'ta' ? 'Add Your First Chart' : 'अपना पहला चार्ट जोड़ें'}
          </button>
        </div>
      ) : (
            ...
            <motion.div key={c.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  {c.is_primary && <span className="text-xs px-1.5 py-0.5 rounded-full bg-gold-primary/20 text-gold-primary font-bold">{locale === 'en' || String(locale) === 'ta' ? 'Primary' : 'प्राथमिक'}</span>}
                </div>
              ...
              <a href={`/${locale}/kundali?n=${encodeURIComponent(c.birth_data.name || c.label)}&d=${c.birth_data.date}&t=${c.birth_data.time}&la=${c.birth_data.lat}&lo=${c.birth_data.lng}&p=${encodeURIComponent(c.birth_data.place)}`}
                ...
                <Eye className="w-3.5 h-3.5" />{locale === 'en' || String(locale) === 'ta' ? 'View Chart' : 'चार्ट देखें'}
              </a>
            </motion.div>
          ))}
        </div>
