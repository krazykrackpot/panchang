#!/usr/bin/env python3
"""
Translate ARCHETYPES (12 lagnas × 6 LocaleText fields) to 7 visible
regional locales via Gemini 2.5 Flash on Vertex AI.

Output: src/lib/constants/archetypes-{locale}-overlay.json, keyed by
"<rashiId>.<field>".
...
JOBS_FILE = Path("/tmp/archetypes-jobs.json")
...
def _write_overlay(locale: str, overlay: dict[str, str]) -> None:
    out_path = OUT_DIR / f"archetypes-{locale}-overlay.json"
    merged: dict[str, str] = {}
    if out_path.exists():
        try:
