...
export default function PanchangPage() {
  ...
  const getNatureLabel = (nature: string) => {
    if (nature === 'auspicious') return L3('Auspicious', 'शुभ', 'சுபம்');
    if (nature === 'inauspicious') return L3('Inauspicious', 'अशुभ', 'அசுபம்');
    return L3('Neutral', 'सम', 'நடுநிலை');
  };
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          ...
          <div className="flex items-center gap-2 rounded-lg border border-gold-primary/12 bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] px-3 py-2">
            ...
            <span className="text-text-primary text-sm font-medium max-w-[180px] truncate">
              {detectingLocation ? (
                <span className="flex items-center gap-1 text-text-secondary text-xs"><Loader2 className="w-3 h-3 animate-spin" />{isTamil ? 'கண்டறிகிறது...' : locale === 'en' ? 'Detecting...' : 'खोज...'}</span>
              ) : location.name}
            </span>
            <button onClick={() => setShowLocationSearch(!showLocationSearch)}
              ...
              {isTamil ? 'மாற்று' : locale === 'en' ? 'Change' : 'बदलें'}
            </button>
          </div>
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex justify-center mt-3">
            <div className="rounded-lg border border-gold-primary/12 bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] p-2 flex gap-2 w-full max-w-sm">
              <input type="text" value={locationInput} onChange={(e) => setLocationInput(e.target.value)}
                ...
                placeholder={isTamil ? 'நகரத்தைத் தேடுங்கள்...' : locale === 'en' ? 'Search city...' : 'शहर खोजें...'}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5 mb-8">
            <div className="flex items-center gap-1.5">
              ...
              <PrintButton
                ...
                label={isTamil ? 'அச்சிடு' : locale === 'en' ? 'Print' : 'प्रिंट'}
                className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-md border border-gold-primary/15 text-text-secondary hover:text-gold-light hover:border-gold-primary/30 transition-all"
              />
              <ShareButton
                title={`Panchang — ${panchang.date}`}
              ...
              <button
                onClick={async () => {
                  ...
                  if (granted) {
                    ...
                    alert(isTamil ? `இன்றுக்கான ${alerts.length} விழிப்பூட்டல்(கள்) திட்டமிடப்பட்டன` : locale === 'en' ? `${alerts.length} alert(s) scheduled for today` : `आज के लिए ${alerts.length} अलर्ट निर्धारित`);
                  } else {
                    alert(isTamil ? 'உலாவி அமைப்புகளில் அறிவிப்புகளை அனுமதிக்கவும்' : locale === 'en' ? 'Please allow notifications in browser settings' : 'कृपया ब्राउज़र सेटिंग्स में सूचनाएं अनुमति दें');
                  }
                }}
                className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-md border border-gold-primary/15 text-text-secondary hover:text-gold-light hover:border-gold-primary/30 transition-all"
              ...
              >
                ...
                {isTamil ? 'விழிப்பூட்டல்கள்' : locale === 'en' ? 'Alerts' : 'अलर्ट'}
              </button>
            </div>
          </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          <span className={`text-xs uppercase tracking-widest font-bold px-2.5 py-0.5 rounded-full ${
                            ...
                            {isMajor
                              ? (isTamil ? 'திருவிழா' : locale === 'en' ? 'Festival' : 'पर्व')
                              : isEkadashi
                              ? (isTamil ? 'ஏகாதசி' : locale === 'en' ? 'Ekadashi' : 'एकादशी')
                              : (isTamil ? 'விரதம்' : locale === 'en' ? 'Vrat' : 'व्रत')}
                          </span>
                        </div>
                        {tl(fest.description) && (
                        ...
                        <div className="flex flex-wrap items-center gap-4 mt-3">
                          ...
                          {fest.paranaStart && (
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-xs text-text-secondary">
                                {isTamil ? 'பாரணை:' : locale === 'en' ? 'Parana:' : 'पारण:'}
                              </span>
          ...
          {(() => {
            ...
            const upto = isTamil ? 'வரை' : locale === 'en' ? 'upto' : 'तक';
            const onwards = isTamil ? 'முதல்' : locale === 'en' ? 'onwards' : 'से आगे';
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-5 mb-14">
                ...
                >
                  ...
                  <div className="text-text-secondary/70 text-xs mt-1.5 leading-snug">
                    {panchang.tithi.paksha === 'shukla'
                      ? (isTamil ? 'சுக்ல பக்ஷம்' : locale === 'en' ? 'Waxing Moon' : 'शुक्ल पक्ष')
                      : (isTamil ? 'கிருஷ்ண பக்ஷம்' : locale === 'en' ? 'Waning Moon' : 'कृष्ण पक्ष')
                    }{' — '}{isTamil ? 'தெய்வம்:' : locale === 'en' ? 'Deity:' : 'देवता:'}{' '}{panchang.tithi.deity[locale]}
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className={`rounded-lg p-2.5 mb-2 border ${nakPassed ? 'border-gold-primary/10 opacity-60' : 'border-gold-primary/30 bg-gold-primary/5'}`}>
                    ...
                    <div className="text-text-secondary text-xs mt-0.5">
                      ...
                      {panchang.nakshatra.pada ? ` · ${isTamil ? 'பாதம்' : locale === 'en' ? 'Pada' : 'पाद'} ${panchang.nakshatra.pada}` : ''}
                    </div>
                    {nakTr && (
                      <div className="mt-2 pt-2 border-t border-gold-primary/10">
                        <div className="font-mono text-sm text-amber-300 font-bold">
                  ...
                  <div className="text-text-secondary/70 text-xs mt-1.5 leading-snug">
                    {isTamil ? 'தன்மை:' : locale === 'en' ? 'Nature:' : 'स्वभाव:'}{' '}{panchang.nakshatra.nature[locale]}{' — '}{isTamil ? 'அதிபதி:' : locale === 'en' ? 'Ruler:' : 'स्वामी:'}{' '}{panchang.nakshatra.rulerName[locale]}
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="text-text-secondary/70 text-xs mt-1.5 leading-snug">
                    {activeYoga.nature === 'auspicious'
                      ? (isTamil ? 'சுபம் — புதிய தொடக்கங்களுக்கு நல்லது' : locale === 'en' ? 'Favorable — good for new beginnings' : 'शुभ — नए कार्यों के लिए अच्छा')
                      : activeYoga.nature === 'inauspicious'
                        ? (isTamil ? 'அசுபம் — முக்கிய பணிகளைத் தவிர்க்கவும்' : locale === 'en' ? 'Unfavorable — avoid important tasks' : 'अशुभ — महत्वपूर्ण कार्य टालें')
                        : (isTamil ? 'நடுநிலை — கவனமாகச் செயல்படவும்' : locale === 'en' ? 'Neutral — proceed with care' : 'सामान्य — सोच-समझकर करें')
                    }
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="text-text-secondary text-xs mt-2">
                    {activeKarana.type === 'chara' ? (isTamil ? 'சர' : locale === 'en' ? 'Movable' : 'चर') : activeKarana.type === 'sthira' ? (isTamil ? 'ஸ்திர' : locale === 'en' ? 'Fixed' : 'स्थिर') : (isTamil ? 'விசேஷ' : locale === 'en' ? 'Special' : 'विशेष')}
                  </div>
                  ...
                  <div className="text-text-secondary/70 text-xs mt-1.5 leading-snug">
                    {activeKarana.type === 'chara'
                      ? (isTamil ? 'சர — பயணம் & புதிய பணிக்கு நல்லது' : locale === 'en' ? 'Movable — good for travel & new work' : 'चर — यात्रा व नए कार्य के लिए')
                      : activeKarana.type === 'sthira'
                        ? (isTamil ? 'ஸ்திர — நிலையான பணிகளுக்கு நல்லது' : locale === 'en' ? 'Fixed — good for stable, lasting work' : 'स्थिर — स्थायी कार्यों के लिए')
                        : (isTamil ? 'விசேஷ கரணம் — எச்சரிக்கையாக இருங்கள்' : locale === 'en' ? 'Special karana — exercise caution' : 'विशेष करण — सावधानी बरतें')
                    }
                  </div>
                </motion.div>
                ...
                >
                  ...
                  {/* Vara contextual tip */}
                  <div className="text-text-secondary/70 text-xs mt-1.5 leading-snug">
                    {isTamil ? 'அதிபதி' : locale === 'en' ? 'Ruled by' : 'स्वामी:'}{' '}{tl(panchang.vara.ruler)}{' — '}{
                      panchang.vara.day === 0 ? (isTamil ? 'ஆன்மீகத்திற்கு நல்லது' : locale === 'en' ? 'good for spirituality' : 'आध्यात्मिक कार्यों के लिए')
                      : panchang.vara.day === 1 ? (isTamil ? 'புதிய தொடக்கங்களுக்கு நல்லது' : locale === 'en' ? 'good for new starts' : 'नई शुरुआत के लिए')
                      : panchang.vara.day === 2 ? (isTamil ? 'தைரியம் & செயலுக்கு நல்லது' : locale === 'en' ? 'good for courage & action' : 'साहस व कार्य के लिए')
                      : panchang.vara.day === 3 ? (isTamil ? 'கல்வி & வணிகத்திற்கு நல்லது' : locale === 'en' ? 'good for learning & trade' : 'विद्या व व्यापार के लिए')
                      : panchang.vara.day === 4 ? (isTamil ? 'வளர்ச்சி & செல்வத்திற்கு நல்லது' : locale === 'en' ? 'good for expansion & wealth' : 'विस्तार व धन के लिए')
                      : panchang.vara.day === 5 ? (isTamil ? 'அழகு & உறவுகளுக்கு நல்லது' : locale === 'en' ? 'good for beauty & relationships' : 'सौंदर्य व संबंधों के लिए')
                      : (isTamil ? 'ஒழுக்கம் & கவனத்திற்கு நல்லது' : locale === 'en' ? 'good for discipline & focus' : 'अनुशासन व एकाग्रता के लिए')
                    }
                  </div>
                </motion.div>
              </div>
          ...
          {panchang.hora && panchang.hora.length > 0 && (() => {
            ...
            const HORA_ACTIVITIES: Record<number, { en: string; hi: string; ta: string }> = {
              0: { en: 'Government work, authority matters, health', hi: 'सरकारी कार्य, अधिकार, स्वास्थ्य', ta: 'அரசு பணி, அதிகாரம், ஆரோக்கியம்' },
              1: { en: 'Travel, liquids, public relations', hi: 'यात्रा, तरल पदार्थ, जनसंपर्क', ta: 'பயணம், திரவங்கள், மக்கள் தொடர்பு' },
              2: { en: 'Property, machinery, legal battles', hi: 'संपत्ति, मशीनरी, कानूनी कार्य', ta: 'சொத்து, இயந்திரம், சட்டப் போராட்டம்' },
              3: { en: 'Communication, trade, learning', hi: 'संचार, व्यापार, शिक्षा', ta: 'தொடர்பு, வணிகம், கல்வி' },
              4: { en: 'Education, finance, spiritual practice', hi: 'शिक्षा, वित्त, आध्यात्मिक साधना', ta: 'கல்வி, நிதி, ஆன்மீக பயிற்சி' },
              5: { en: 'Romance, arts, luxury purchases', hi: 'प्रेम, कला, विलासिता', ta: 'காதல், கலை, ஆடம்பரம்' },
              6: { en: 'Labor, iron work, mining, discipline', hi: 'श्रम, लोहा, खनन, अनुशासन', ta: 'உழைப்பு, இரும்புப் பணி, சுரங்கம், ஒழுக்கம்' },
            };
            ...
            return (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                ...
                <div className="text-gold-dark text-xs uppercase tracking-widest font-bold mb-2">
                  {isTamil ? 'தற்போதைய ஹோரை — இப்போது சிறந்த செயல்' : locale === 'en' ? 'Current Hora — Best Activity Now' : 'वर्तमान होरा — अभी सर्वोत्तम कार्य'}
                </div>
                ...
                <div className="text-text-secondary text-sm mt-1.5" style={isDevanagari ? { fontFamily: 'var(--font-devanagari-body)' } : undefined}>
                  {isTamil ? activity.ta : locale === 'en' ? activity.en : activity.hi}
                </div>
              </motion.div>
            );
          })()}
          ...
          <div className="my-14">
            <SectionHeading
              ...
              title={isTamil ? 'சுப நேரங்கள்' : locale === 'en' ? 'Auspicious Timings' : 'शुभ मुहूर्त'}
              subtitle={isTamil
              ...
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex justify-center">
                <div className="flex items-center gap-2 px-5 py-2.5 rounded-full border border-amber-400/40 bg-amber-500/10">
                  ...
                  <span className="text-amber-300 text-sm font-bold uppercase tracking-wider">
                    {isTamil ? 'ரவி யோகம்' : locale === 'en' ? 'Ravi Yoga' : 'रवि योग'}
                  </span>
                  ...
                  <span className="text-amber-400/70 text-xs">{isTamil ? '· மிகவும் சுபம்' : locale === 'en' ? '· Highly Auspicious' : '· अत्यंत शुभ'}</span>
                </div>
              </motion.div>
            )}
                ...
                <div className="rounded-2xl border-2 border-emerald-500/40 bg-gradient-to-r from-emerald-500/10 via-gold-primary/5 to-emerald-500/10 p-4 text-center">
                  <div className="flex items-center justify-center gap-2 mb-1">
                    ...
                    <span className="text-emerald-400 text-xs uppercase tracking-[0.3em] font-bold">
                      {isTamil ? 'அமிர்த சித்தி யோகம்' : locale === 'en' ? 'Amrit Siddhi Yoga' : 'अमृत सिद्धि योग'}
                    </span>
                    <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                  </div>
                  <div className="text-emerald-300 text-base font-bold">
                    {isTamil ? 'மிக உயர்ந்த சுப நாள்' : locale === 'en' ? 'Supremely Auspicious Day' : 'अत्यंत शुभ दिन'}
                  </div>
              ...
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex justify-center">
                <div className="flex items-center gap-2 px-5 py-2.5 rounded-full border border-rose-500/40 bg-rose-500/10">
                  ...
                  <span className="text-rose-400 text-sm font-bold uppercase tracking-wider">
                    {isTamil ? 'தக்த திதி' : locale === 'en' ? 'Dagdha Tithi' : 'दग्ध तिथि'}
                  </span>
                  <span className="text-rose-400/70 text-xs">
                    {isTamil ? '· புதிய தொடக்கங்களைத் தவிர்க்கவும்' : locale === 'en' ? '· Avoid new beginnings' : '· नए कार्य वर्जित'}
                  </span>
                </div>
              </motion.div>
            )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
                ...
                <div className="text-gold-primary text-xs uppercase tracking-wider font-bold mb-2">
                  {isTamil ? 'அபிஜித் முகூர்த்தம்' : locale === 'en' ? 'Abhijit Muhurta' : 'अभिजित् मुहूर्त'}
                </div>
                ...
                <div className="text-text-secondary text-xs mt-2 leading-relaxed">
                  {isTamil ? 'மிகவும் சுபம் — வெற்றி உறுதி' : locale === 'en' ? 'Most auspicious — victory assured' : 'सर्वश्रेष्ठ — विजय निश्चित'}
                </div>
              </motion.div>
              ...
              {panchang.vijayaMuhurta && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.11 }}
                  ...
                  <div className="text-amber-400 text-xs uppercase tracking-wider font-bold mb-2">
                    {isTamil ? 'விஜய முகூர்த்தம்' : locale === 'en' ? 'Vijaya Muhurta' : 'विजय मुहूर्त'}
                  </div>
                  <div className="font-mono text-xl font-bold text-amber-300">{panchang.vijayaMuhurta.start} — {panchang.vijayaMuhurta.end}</div>
                  <div className="text-text-secondary text-xs mt-2 leading-relaxed">
                    {isTamil ? '10வது முகூர்த்தம் — வெற்றி உறுதி' : locale === 'en' ? '10th muhurta — victory assured' : '१०वाँ मुहूर्त — विजय निश्चित'}
                  </div>
                </motion.div>
              )}
              ...
              {panchang.sandhyaKaal && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.20 }}
                  ...
                  <div className="text-orange-400 text-xs uppercase tracking-wider font-bold mb-2">
                    {isTamil ? 'காலை சந்தியா' : locale === 'en' ? 'Morning Sandhya' : 'प्रातः संध्या'}
                  </div>
                  ...
                  <div className="text-text-secondary text-xs mt-2 leading-relaxed">
                    {isTamil ? 'பிராதஃ சந்தியா — காலை வழிபாடு' : locale === 'en' ? 'Pratah Sandhya — morning prayers' : 'प्रातः संध्या — प्रातःकालीन पूजा'}
                  </div>
                </motion.div>
              )}
              ...
              {panchang.sandhyaKaal && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.23 }}
                  ...
                  <div className="text-purple-400 text-xs uppercase tracking-wider font-bold mb-2">
                    {isTamil ? 'மாலை சந்தியா' : locale === 'en' ? 'Evening Sandhya' : 'सायं संध्या'}
                  </div>
                  ...
                  <div className="text-text-secondary text-xs mt-2 leading-relaxed">
                    {isTamil ? 'சாயங்கால சந்தியா — மாலை வழிபாடு' : locale === 'en' ? 'Sayahna Sandhya — evening prayers' : 'सायंकाल संध्या — सन्ध्याकालीन पूजा'}
                  </div>
                </motion.div>
              )}
              ...
              {panchang.anandadiYoga && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.29 }}
                  ...
                  <div className={`text-xs uppercase tracking-wider font-bold mb-2 ${panchang.anandadiYoga.nature === 'auspicious' ? 'text-emerald-400' : 'text-orange-400'}`}>
                    {isTamil ? 'ஆனந்தாதி யோகம்' : locale === 'en' ? 'Anandadi Yoga' : 'आनन्दादि योग'}
                  </div>
                  ...
                  <div className="text-text-secondary text-xs mt-2 leading-relaxed">
                    {panchang.anandadiYoga.nature === 'auspicious'
                      ? (isTamil ? 'சுப யோகம் — நல்ல ஆற்றல்' : locale === 'en' ? 'Auspicious yoga — favourable energy' : 'शुभ योग — अनुकूल ऊर्जा')
                      : (isTamil ? 'கலப்பு ஆற்றல் — எச்சரிக்கையாகச் செயல்படவும்' : locale === 'en' ? 'Mixed energy — act with caution' : 'मिश्रित ऊर्जा — सावधानी से कार्य करें')}
                  </div>
                </motion.div>
              )}
              ...
              {panchang.tamilYoga && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.35 }}
                  ...
                  <div className={`text-xs uppercase tracking-wider font-bold mb-2 ${panchang.tamilYoga.nature === 'auspicious' ? 'text-emerald-400' : 'text-red-400'}`}>
                    {isTamil ? 'தமிழ் யோகம்' : locale === 'en' ? 'Tamil Yoga' : 'तमिल योग'}
                  </div>
                  ...
                  <div className="text-text-secondary text-xs mt-2">
                    {panchang.tamilYoga.nature === 'auspicious'
                      ? (isTamil ? 'சுபம் — முக்கிய செயல்களுக்கு நல்லது' : locale === 'en' ? 'Auspicious — good for important activities' : 'शुभ — महत्वपूर्ण कार्यों के लिए उत्तम')
                      : (isTamil ? 'அசுபம் — புதிய முயற்சிகளைத் தவிர்க்கவும்' : locale === 'en' ? 'Inauspicious — avoid new ventures' : 'अशुभ — नए कार्यों से बचें')}
                  </div>
                </motion.div>
              )}
              ...
              {panchang.homahuti && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4 }}
                  ...
                  <div className="text-orange-400 text-xs uppercase tracking-wider font-bold mb-2">
                    {isTamil ? 'ஹோமாஹுதி திசை' : locale === 'en' ? 'Homahuti Direction' : 'होमाहुति दिशा'}
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-text-secondary text-xs">{isTamil ? 'ஹோமத்திற்கு இத்திசையில் முகம் செய்யவும்' : locale === 'en' ? 'Face this direction for Homa' : 'होम के लिए इस दिशा में मुख करें'}</div>
                    </div>
                  </div>
                  <div className="text-text-tertiary text-xs mt-2">
                    {isTamil ? `அதிதேவதை: ${panchang.homahuti.deity.en}` : locale === 'en' ? `Presiding deity: ${panchang.homahuti.deity.en}` : `अधिष्ठाता: ${panchang.homahuti.deity.hi}`}
                  </div>
                </motion.div>
              )}
              ...
              {panchang.mantriMandala && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.45 }}
                  ...
                  <div className="text-gold-dark text-xs uppercase tracking-wider font-bold mb-3">
                    {isTamil ? 'மந்திரி மண்டலம் & ஹோரை' : locale === 'en' ? 'Mantri Mandala & Hora' : 'मंत्री मण्डल एवं होरा'}
                  </div>
                  <div className="space-y-2 mb-3">
                    <div className="flex items-center justify-between">
                      <span className="text-text-secondary text-xs">{isTamil ? 'நாள் அதிபதி (அரசர்)' : locale === 'en' ? 'Day Lord (King)' : 'दिन स्वामी (राजा)'}</span>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-text-secondary text-xs">{isTamil ? 'மத்தியான ஹோரை அதிபதி (மந்திரி)' : locale === 'en' ? 'Midday Hora Lord (Minister)' : 'मध्याह्न होरा स्वामी (मंत्री)'}</span>
                  ...
                  <div className="text-text-tertiary text-xs mb-3">
                    {isTamil ? 'நாள் அதிபதி முழு நாளையும் ஆளுகிறார். ஒவ்வொரு ஹோரையும் (~1 மணி நேரம்) கால்டியன் வரிசையில் வேறு கிரகம் ஆளுகிறது.' : locale === 'en' ? 'Day lord rules the entire day. Each hora (~1 hour) is ruled by a different planet in Chaldean sequence.' : 'दिन स्वामी सम्पूर्ण दिन का शासक। प्रत्येक होरा (~1 घंटा) कल्डियन क्रम में भिन्न ग्रह द्वारा शासित।'}
                  </div>
                  ...
                  {panchang.mantriMandala.horas && (() => {
                        ...
                        <div key={i} className={`flex flex-col py-0.5 border-b border-gold-primary/5 ${hNow ? 'bg-gold-primary/10 rounded px-1 -mx-1 font-bold' : ''}`}>
                          ...
                          {hasConflict && (
                            <span className="text-amber-400/80 text-[9px] leading-tight">
                              {isTamil ? conflicts.map(c => c.label).join(', ') : locale === 'en' ? conflicts.map(c => c.label).join(', ') : conflicts.map(c => c.labelHi).join(', ')}
                            </span>
                          )}
                        </div>
                      );
                    ...
                    <details className="group">
                      <summary className="text-gold-primary/70 text-xs cursor-pointer hover:text-gold-light transition-colors">
                        {isTamil ? '▸ அனைத்து 24 ஹோரைகளையும் காண்க' : locale === 'en' ? '▸ View all 24 Horas' : '▸ सभी 24 होरा देखें'}
                      </summary>
                      <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-0.5 text-xs">
                        <div>
                          <div className="text-gold-dark font-bold text-[10px] uppercase tracking-wider mb-1">{isTamil ? 'பகல் ஹோரைகள்' : locale === 'en' ? 'Day Horas' : 'दिन होरा'}</div>
                        ...
                        <div>
                          <div className="text-gold-dark font-bold text-[10px] uppercase tracking-wider mb-1">{isTamil ? 'இரவு ஹோரைகள்' : locale === 'en' ? 'Night Horas' : 'रात्रि होरा'}</div>
                          {panchang.mantriMandala.horas.filter((h: { isDay: boolean }) => !h.isDay).map((h: { planet: number; start: string; end: string }, i: number) => renderHora(h, i))}
                        </div>
                      </div>
                    </details>
          ...
          <div className="my-14">
            <SectionHeading
              ...
              title={isTamil ? 'அசுப காலங்கள்' : locale === 'en' ? 'Inauspicious Timings' : 'अशुभ काल'}
              subtitle={isTamil
                ? 'இந்த கிரக தோஷ காலங்களில் புதிய செயல்கள், பயணங்கள் அல்லது முக்கிய முடிவுகளைத் தொடங்குவதைத் தவிர்க்கவும்.'
                : locale === 'en'
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.05 }}
                ...
                <div className="text-text-secondary text-xs mt-2">{isTamil ? 'ராகு காலம் — புதிய பணிகளைத் தவிர்க்கவும்' : locale === 'en' ? 'Rahu\'s period — avoid new work' : 'राहु काल — नए कार्य टालें'}</div>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.08 }}
                ...
                <div className="text-text-secondary text-xs mt-2">{isTamil ? 'எமகண்டம் — அசுபம்' : locale === 'en' ? 'Yama\'s period — inauspicious' : 'यम का काल — अशुभ'}</div>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.11 }}
                ...
                <div className="text-text-secondary text-xs mt-2">{isTamil ? 'குளிகை காலம் — பயணம் தவிர்க்கவும்' : locale === 'en' ? 'Gulika\'s period — avoid travel' : 'गुलिक काल — यात्रा टालें'}</div>
              </motion.div>
              ...
              {panchang.durMuhurtam && panchang.durMuhurtam.length > 0 && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.14 }}
                  ...
                  <div className="text-red-500 text-xs uppercase tracking-widest mb-2 font-bold">
                    {isTamil ? 'துர்முகூர்த்தம்' : locale === 'en' ? 'Dur Muhurtam' : 'दुर्मुहूर्त'}
                  </div>
                  <div className="text-text-secondary/50 text-[10px] mb-2">
                    {isTamil ? 'கால பிரகாசிகா மரபு' : locale === 'en' ? 'Kaala Prakashika tradition' : 'काल प्रकाशिका परम्परा'}
                  </div>
                  ...
                  {panchang.durMuhurtamAlt && panchang.durMuhurtamAlt.length > 0 && (
                    <details className="mt-3 text-left">
                      <summary className="text-text-tertiary/50 text-[10px] cursor-pointer hover:text-text-secondary transition-colors text-center">
                        {isTamil ? '▸ நிர்ணய சிந்து மரபு (மாற்று)' : locale === 'en' ? '▸ Nirṇaya Sindhu tradition (alternate)' : '▸ निर्णय सिन्धु परम्परा (वैकल्पिक)'}
                      </summary>
                  ...
                  <div className="text-text-secondary text-xs mt-2">{isTamil ? 'அசுபம் — அனைத்து புதிய பணிகளையும் தவிர்க்கவும்' : locale === 'en' ? 'Inauspicious — avoid all new work' : 'अशुभ — सभी नए कार्य टालें'}</div>
                </motion.div>
              )}
              ...
              {panchang.vishaGhatika && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.15 }}
                  ...
                  <div className="text-red-500 text-xs uppercase tracking-widest mb-2 font-bold">
                    {isTamil ? 'விஷ கடிகை' : locale === 'en' ? 'Visha Ghatika' : 'विष घटी'}
                  </div>
                  <div className="font-mono text-lg font-bold text-red-400">{panchang.vishaGhatika.start} — {panchang.vishaGhatika.end}</div>
                  <div className="text-text-secondary text-xs mt-2">
                    {isTamil ? '25வது கடிகை — விஷ காலம், அனைத்து புதிய செயல்களையும் தவிர்க்கவும்' : locale === 'en' ? '25th Ghatika — poison period, avoid all new activities' : '25वीं घटी — विष काल, सभी नए कार्य वर्जित'}
                  </div>
                </motion.div>
              )}
              ...
              {panchang.bhadra && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.18 }}
                  ...
                  <div className="text-orange-400 text-xs uppercase tracking-widest mb-2 font-bold">
                    {isTamil ? 'பத்ரா (விஷ்டி)' : locale === 'en' ? 'Bhadra (Vishti)' : 'भद्रा (विष्टि)'}
                  </div>
                  ...
                  <div className="text-text-secondary text-xs mt-2">
                    {isTamil ? 'அசுப கரணம் — முக்கிய செயல்களைத் தவிர்க்கவும்' : locale === 'en' ? 'Inauspicious karana — avoid important activities' : 'अशुभ करण — महत्वपूर्ण कार्यों से बचें'}
                  </div>
                </motion.div>
              )}
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.20 }}
                  ...
                  <div className="flex items-center justify-center gap-2 mb-2">
                    ...
                    <div className="text-red-400 text-xs uppercase tracking-widest font-bold">
                      {isTamil ? 'கண்டமூலம்' : locale === 'en' ? 'Ganda Moola' : 'गण्डमूल'}
                    </div>
                    <span className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse" />
                  </div>
                  ...
                  <div className="text-text-secondary text-xs mt-2">
                    {isTamil ? 'அசுப நட்சத்திர சந்தி — எச்சரிக்கையாக இருங்கள்' : locale === 'en' ? 'Inauspicious nakshatra junction — exercise caution' : 'अशुभ नक्षत्र संधि — सावधानी बरतें'}
                  </div>
                </motion.div>
              )}
              ...
              {panchang.aadalYoga && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.21 }}
                  ...
                  <div className="text-amber-400 text-xs uppercase tracking-widest mb-2 font-bold">
                    {isTamil ? 'ஆடல் யோகம்' : locale === 'en' ? 'Aadal Yoga' : 'आदल योग'}
                  </div>
                  ...
                  <div className="text-text-secondary text-xs mt-2">
                    {isTamil ? 'கண்டமூலத்திற்குப் பின் சந்திகாலம் — அசுப மாற்றம்' : locale === 'en' ? 'Post-Ganda Moola junction — inauspicious transition' : 'गण्डमूल उपरांत संधिकाल — अशुभ संक्रमण'}
                  </div>
                </motion.div>
              )}
              ...
              {panchang.vidaalYoga && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.22 }}
                  ...
                  <div className="text-rose-400 text-xs uppercase tracking-widest mb-2 font-bold">
                    {isTamil ? 'விடால யோகம்' : locale === 'en' ? 'Vidaal Yoga' : 'विदाल योग'}
                  </div>
                  ...
                  <div className="text-text-secondary text-xs mt-2">
                    {isTamil ? 'அசுப சந்திர-நட்சத்திர யோகம் — புதிய முயற்சிகளைத் தவிர்க்கவும்' : locale === 'en' ? 'Malefic Moon-nakshatra yoga — avoid new ventures' : 'अशुभ चन्द्र-नक्षत्र योग — नए कार्य टालें'}
                  </div>
                </motion.div>
              )}
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.23 }}
                  ...
                  <div className="flex items-center justify-center gap-2 mb-2">
                    ...
                    <div className="text-purple-400 text-xs uppercase tracking-widest font-bold">
                      {isTamil ? 'பஞ்சகம்' : locale === 'en' ? 'Panchaka' : 'पंचक'}
                    </div>
                    <span className="w-2.5 h-2.5 rounded-full bg-purple-500 animate-pulse" />
                  </div>
                  ...
                  <div className="text-text-secondary text-xs mt-2">
                    {isTamil ? 'ஐந்து மடங்கு தோஷம் — தீ, கட்டுமானம், பயணம் தவிர்க்கவும்' : locale === 'en' ? 'Five-fold affliction — avoid fire, construction, travel' : 'पंचगुना पीड़ा — अग्नि, निर्माण, यात्रा टालें'}
                  </div>
                </motion.div>
              )}
            </div>
          ...
          <div className="my-14">
            <SectionHeading
              ...
              title={isTamil ? 'நிவாசம் & சூலம்' : locale === 'en' ? 'Nivas & Shool' : 'निवास एवं शूल'}
              subtitle={isTamil
                ? 'இன்று அண்ட சக்திகள் எங்கு வசிக்கின்றன — திசை தோஷங்கள், தெய்வீக நிவாசங்கள், பரிகாரங்கள்.'
                : locale === 'en'
            ...
            <div className="text-center mb-6">
              <Link href="/nivas-shool" className="inline-flex items-center gap-1.5 text-sm text-indigo-400 hover:text-indigo-300 transition-colors">
                {isTamil ? 'அனைத்து நிவாசம் & சூல கருத்துகளைப் பற்றி அறிக' : locale === 'en' ? 'Learn about all Nivas & Shool concepts' : 'सभी निवास एवं शूल अवधारणाओं के बारे में जानें'} →
              </Link>
            </div>
              ...
              {panchang.shivaVaas && (() => {
                ...
                const shivaDesc: Record<string, { nColor: string; border: string; bg: string; desc: LocaleText; activities: LocaleText }> = {
                  'Kailash (Mountain)': {
                    ...
                    desc: { en: 'Shiva resides in his divine abode atop Kailash with Parvati. He is serene, benevolent, and easily pleased. Best time for Shiva puja, abhishek, and auspicious activities.', hi: 'शिव कैलाश पर पार्वती के साथ हैं। वे शांत, दयालु और प्रसन्न हैं। शिव पूजा, अभिषेक और शुभ कार्यों का सर्वोत्तम समय।', sa: 'शिव कैलाश पर पार्वती के साथ हैं। वे शांत, दयालु और प्रसन्न हैं। शिव पूजा, अभिषेक और शुभ कार्यों का सर्वोत्तम समय।', mai: 'शिव कैलाश पर पार्वती के साथ हैं। वे शांत, दयालु और प्रसन्न हैं। शिव पूजा, अभिषेक और शुभ कार्यों का सर्वोत्तम समय।', mr: 'शिव कैलाश पर पार्वती के साथ हैं। वे शांत, दयालु और प्रसन्न हैं। शिव पूजा, अभिषेक और शुभ कार्यों का सर्वोत्तम समय।', ta: 'Shiva resides in his divine abode atop Kailash with Parvati. He is serene, benevolent, and easily pleased. Best time for Shiva puja, abhishek, and auspicious activities.', te: 'Shiva resides in his divine abode atop Kailash with Parvati. He is serene, benevolent, and easily pleased. Best time for Shiva puja, abhishek, and auspicious activities.', bn: 'Shiva resides in his divine abode atop Kailash with Parvati. He is serene, benevolent, and easily pleased. Best time for Shiva puja, abhishek, and auspicious activities.', kn: 'Shiva resides in his divine abode atop Kailash with Parvati. He is serene, benevolent, and easily pleased. Best time for Shiva puja, abhishek, and auspicious activities.', gu: 'Shiva resides in his divine abode atop Kailash with Parvati. He is serene, benevolent, and easily pleased. Best time for Shiva puja, abhishek, and auspicious activities.' },
                    activities: { en: 'Shiva puja, abhishek, marriage, business launch', hi: 'शिव पूजा, अभिषेक, विवाह, व्यापार आरंभ', sa: 'शिव पूजा, अभिषेक, विवाह, व्यापार आरंभ', mai: 'शिव पूजा, अभिषेक, विवाह, व्यापार आरंभ', mr: 'शिव पूजा, अभिषेक, विवाह, व्यापार आरंभ', ta: 'Shiva puja, abhishek, marriage, business launch', te: 'Shiva puja, abhishek, marriage, business launch', bn: 'Shiva puja, abhishek, marriage, business launch', kn: 'Shiva puja, abhishek, marriage, business launch', gu: 'Shiva puja, abhishek, marriage, business launch' },
                  },
                  'Shamshan (Cremation Ground)': {
                    nColor: 'text-red-400', border: 'border-red-500/25', bg: 'from-red-500/5',
                    desc: { en: 'Shiva dwells in the cremation ground in his fierce Rudra/Bhairava form. Auspicious activities should be avoided; Tantric rites may be performed by adepts.', hi: 'शिव श्मशान में रुद्र/भैरव रूप में हैं। शुभ कार्यों से बचें; तांत्रिक अनुष्ठान साधक कर सकते हैं।', sa: 'शिव श्मशान में रुद्र/भैरव रूप में हैं। शुभ कार्यों से बचें; तांत्रिक अनुष्ठान साधक कर सकते हैं।', mai: 'शिव श्मशान में रुद्र/भैरव रूप में हैं। शुभ कार्यों से बचें; तांत्रिक अनुष्ठान साधक कर सकते हैं।', mr: 'शिव श्मशान में रुद्र/भैरव रूप में हैं। शुभ कार्यों से बचें; तांत्रिक अनुष्ठान साधक कर सकते हैं।', ta: 'Shiva dwells in the cremation ground in his fierce Rudra/Bhairava form. Auspicious activities should be avoided; Tantric rites may be performed by adepts.', te: 'Shiva dwells in the cremation ground in his fierce Rudra/Bhairava form. Auspicious activities should be avoided; Tantric rites may be performed by adepts.', bn: 'Shiva dwells in the cremation ground in his fierce Rudra/Bhairava form. Auspicious activities should be avoided; Tantric rites may be performed by adepts.', kn: 'Shiva dwells in the cremation ground in his fierce Rudra/Bhairava form. Auspicious activities should be avoided; Tantric rites may be performed by adepts.', gu: 'Shiva dwells in the cremation ground in his fierce Rudra/Bhairava form. Auspicious activities should be avoided; Tantric rites may be performed by adepts.' },
                    activities: { en: 'Avoid auspicious events; Rudra worship, ancestral rites', hi: 'शुभ कार्य वर्जित; रुद्र पूजा, पित्र कार्य', sa: 'शुभ कार्य वर्जित; रुद्र पूजा, पित्र कार्य', mai: 'शुभ कार्य वर्जित; रुद्र पूजा, पित्र कार्य', mr: 'शुभ कार्य वर्जित; रुद्र पूजा, पित्र कार्य', ta: 'Avoid auspicious events; Rudra worship, ancestral rites', te: 'Avoid auspicious events; Rudra worship, ancestral rites', bn: 'Avoid auspicious events; Rudra worship, ancestral rites', kn: 'Avoid auspicious events; Rudra worship, ancestral rites', gu: 'Avoid auspicious events; Rudra worship, ancestral rites' },
                  },
                  "Gori's Abode (Auspicious)": {
                    ...
                    activities: { en: 'Marriage, family rituals, home purchase, Gauri puja', hi: 'विवाह, पारिवारिक अनुष्ठान, गृह क्रय, गौरी पूजा', sa: 'विवाह, पारिवारिक अनुष्ठान, गृह क्रय, गौरी पूजा', mai: 'विवाह, पारिवारिक अनुष्ठान, गृह क्रय, गौरी पूजा', mr: 'विवाह, पारिवारिक अनुष्ठान, गृह क्रय, गौरी पूजा', ta: 'Marriage, family rituals, home purchase, Gauri puja', te: 'Marriage, family rituals, home purchase, Gauri puja', bn: 'Marriage, family rituals, home purchase, Gauri puja', kn: 'Marriage, family rituals, home purchase, Gauri puja', gu: 'Marriage, family rituals, home purchase, Gauri puja' },
                  },
                  'Sports & Play': {
                    ...
                    activities: { en: 'Arts, music, dance; avoid critical ventures', hi: 'कला, संगीत, नृत्य; महत्वपूर्ण कार्यों से बचें', sa: 'कला, संगीत, नृत्य; महत्वपूर्ण कार्यों से बचें', mai: 'कला, संगीत, नृत्य; महत्वपूर्ण कार्यों से बचें', mr: 'कला, संगीत, नृत्य; महत्वपूर्ण कार्यों से बचें', ta: 'Arts, music, dance; avoid critical ventures', te: 'Arts, music, dance; avoid critical ventures', bn: 'Arts, music, dance; avoid critical ventures', kn: 'Arts, music, dance; avoid critical ventures', gu: 'Arts, music, dance; avoid critical ventures' },
                  },
                  'Deep Meditation (Samadhi)': {
                    nColor: 'text-violet-300', border: 'border-violet-500/25', bg: 'from-violet-500/5',
                    desc: { en: 'Shiva is in deep Samadhi — beyond the phenomenal world. Sacred for meditation and spiritual practice, but mundane activities may not receive divine support.', hi: 'शिव गहरी समाधि में हैं। ध्यान और आध्यात्मिक साधना के लिए पवित्र, लेकिन सांसारिक कार्यों में कम सहायता।', sa: 'शिव गहरी समाधि में हैं। ध्यान और आध्यात्मिक साधना के लिए पवित्र, लेकिन सांसारिक कार्यों में कम सहायता।', mai: 'शिव गहरी समाधि में हैं। ध्यान और आध्यात्मिक साधना के लिए पवित्र, लेकिन सांसारिक कार्यों में कम सहायता।', mr: 'शिव गहरी समाधि में हैं। ध्यान और आध्यात्मिक साधना के लिए पवित्र, लेकिन सांसारिक कार्यों में कम सहायता।', ta: 'Shiva is in deep Samadhi — beyond the phenomenal world. Sacred for meditation and spiritual practice, but mundane activities may not receive divine support.', te: 'Shiva is in deep Samadhi — beyond the phenomenal world. Sacred for meditation and spiritual practice, but mundane activities may not receive divine support.', bn: 'Shiva is in deep Samadhi — beyond the phenomenal world. Sacred for meditation and spiritual practice, but mundane activities may not receive divine support.', kn: 'Shiva is in deep Samadhi — beyond the phenomenal world. Sacred for meditation and spiritual practice, but mundane activities may not receive divine support.', gu: 'Shiva is in deep Samadhi — beyond the phenomenal world. Sacred for meditation and spiritual practice, but mundane activities may not receive divine support.' },
                    activities: { en: 'Meditation, japa, fasting. Avoid worldly ventures.', hi: 'ध्यान, जप, उपवास। सांसारिक कार्यों से बचें।', sa: 'ध्यान, जप, उपवास। सांसारिक कार्यों से बचें।', mai: 'ध्यान, जप, उपवास। सांसारिक कार्यों से बचें।', mr: 'ध्यान, जप, उपवास। सांसारिक कार्यों से बचें।', ta: 'Meditation, japa, fasting. Avoid worldly ventures.', te: 'Meditation, japa, fasting. Avoid worldly ventures.', bn: 'Meditation, japa, fasting. Avoid worldly ventures.', kn: 'Meditation, japa, fasting. Avoid worldly ventures.', gu: 'Meditation, japa, fasting. Avoid worldly ventures.' },
                  },
                };
              ...
              {panchang.agniVaas && (() => {
                ...
                const agniDesc: Record<string, { nColor: string; border: string; bg: string; desc: LocaleText; ritualNote: LocaleText }> = {
                  'Sky (Akasha)': {
                    ...
                    desc: { en: 'Agni resides in the celestial sky. Fire rituals are carried directly to the devatas.', hi: 'अग्नि आकाश में हैं। अग्नि अनुष्ठान सीधे देवताओं तक पहुँचते हैं।', sa: 'अग्नि आकाश में हैं। अग्नि अनुष्ठान सीधे देवताओं तक पहुँचते हैं।', mai: 'अग्नि आकाश में हैं। अग्नि अनुष्ठान सीधे देवताओं तक पहुँचते हैं।', mr: 'अग्नि आकाश में हैं। अग्नि अनुष्ठान सीधे देवताओं तक पहुँचते हैं।', ta: 'Agni resides in the celestial sky. Fire rituals are carried directly to the devatas.', te: 'Agni resides in the celestial sky. Fire rituals are carried directly to the devatas.', bn: 'Agni resides in the celestial sky. Fire rituals are carried directly to the devatas.', kn: 'Agni resides in the celestial sky. Fire rituals are carried directly to the devatas.', gu: 'Agni resides in the celestial sky. Fire rituals are carried directly to the devatas.' },
                    ritualNote: { en: 'Homa & Yajna highly effective — offerings rise unimpeded', hi: 'होम और यज्ञ अत्यंत प्रभावी — आहुतियाँ बाधारहित ऊर्ध्वगामी', sa: 'होम और यज्ञ अत्यंत प्रभावी — आहुतियाँ बाधारहित ऊर्ध्वगामी', mai: 'होम और यज्ञ अत्यंत प्रभावी — आहुतियाँ बाधारहित ऊर्ध्वगामी', mr: 'होम और यज्ञ अत्यंत प्रभावी — आहुतियाँ बाधारहित ऊर्ध्वगामी', ta: 'Homa & Yajna highly effective — offerings rise unimpeded', te: 'Homa & Yajna highly effective — offerings rise unimpeded', bn: 'Homa & Yajna highly effective — offerings rise unimpeded', kn: 'Homa & Yajna highly effective — offerings rise unimpeded', gu: 'Homa & Yajna highly effective — offerings rise unimpeded' },
                  },
                  'Earth (Bhumi)': {
                    nColor: 'text-emerald-300', border: 'border-emerald-500/25', bg: 'from-emerald-500/5',
                    desc: { en: 'Agni is grounded in the earth plane. Fire rituals nourish the land and people.', hi: 'अग्नि पृथ्वी पर हैं। अग्नि अनुष्ठान भूमि और लोगों को पोषित करते हैं।', sa: 'अग्नि पृथ्वी पर हैं। अग्नि अनुष्ठान भूमि और लोगों को पोषित करते हैं।', mai: 'अग्नि पृथ्वी पर हैं। अग्नि अनुष्ठान भूमि और लोगों को पोषित करते हैं।', mr: 'अग्नि पृथ्वी पर हैं। अग्नि अनुष्ठान भूमि और लोगों को पोषित करते हैं।', ta: 'Agni is grounded in the earth plane. Fire rituals nourish the land and people.', te: 'Agni is grounded in the earth plane. Fire rituals nourish the land and people.', bn: 'Agni is grounded in the earth plane. Fire rituals nourish the land and people.', kn: 'Agni is grounded in the earth plane. Fire rituals nourish the land and people.', gu: 'Agni is grounded in the earth plane. Fire rituals nourish the land and people.' },
                    ritualNote: { en: 'Agriculture blessings, prosperity rituals, Griha Pravesh powerful', hi: 'कृषि, समृद्धि, गृह प्रवेश अग्नि कार्य विशेष प्रभावी', sa: 'कृषि, समृद्धि, गृह प्रवेश अग्नि कार्य विशेष प्रभावी', mai: 'कृषि, समृद्धि, गृह प्रवेश अग्नि कार्य विशेष प्रभावी', mr: 'कृषि, समृद्धि, गृह प्रवेश अग्नि कार्य विशेष प्रभावी', ta: 'Agriculture blessings, prosperity rituals, Griha Pravesh powerful', te: 'Agriculture blessings, prosperity rituals, Griha Pravesh powerful', bn: 'Agriculture blessings, prosperity rituals, Griha Pravesh powerful', kn: 'Agriculture blessings, prosperity rituals, Griha Pravesh powerful', gu: 'Agriculture blessings, prosperity rituals, Griha Pravesh powerful' },
                  },
                  'Patala': {
                    nColor: 'text-red-400', border: 'border-red-500/25', bg: 'from-red-500/5',
                    desc: { en: 'Agni descends to the netherworld. Fire rituals may have reversed or weakened effects.', hi: 'अग्नि पाताल में हैं। अग्नि अनुष्ठानों के उलटे या कमजोर प्रभाव।', sa: 'अग्नि पाताल में हैं। अग्नि अनुष्ठानों के उलटे या कमजोर प्रभाव।', mai: 'अग्नि पाताल में हैं। अग्नि अनुष्ठानों के उलटे या कमजोर प्रभाव।', mr: 'अग्नि पाताल में हैं। अग्नि अनुष्ठानों के उलटे या कमजोर प्रभाव।', ta: 'Agni descends to the netherworld. Fire rituals may have reversed or weakened effects.', te: 'Agni descends to the netherworld. Fire rituals may have reversed or weakened effects.', bn: 'Agni descends to the netherworld. Fire rituals may have reversed or weakened effects.', kn: 'Agni descends to the netherworld. Fire rituals may have reversed or weakened effects.', gu: 'Agni descends to the netherworld. Fire rituals may have reversed or weakened effects.' },
                    ritualNote: { en: 'Major Yajnas should be postponed. Simple Deepa worship permitted.', hi: 'बड़े यज्ञ स्थगित करें। साधारण दीप पूजा कर सकते हैं।', sa: 'बड़े यज्ञ स्थगित करें। साधारण दीप पूजा कर सकते हैं।', mai: 'बड़े यज्ञ स्थगित करें। साधारण दीप पूजा कर सकते हैं।', mr: 'बड़े यज्ञ स्थगित करें। साधारण दीप पूजा कर सकते हैं।', ta: 'Major Yajnas should be postponed. Simple Deepa worship permitted.', te: 'Major Yajnas should be postponed. Simple Deepa worship permitted.', bn: 'Major Yajnas should be postponed. Simple Deepa worship permitted.', kn: 'Major Yajnas should be postponed. Simple Deepa worship permitted.', gu: 'Major Yajnas should be postponed. Simple Deepa worship permitted.' },
                  },
                  'Water (Jal)': {
                    nColor: 'text-blue-300', border: 'border-blue-500/25', bg: 'from-blue-500/5',
                    desc: { en: 'Agni is submerged in water — the Vadavagni (submarine fire of Hindu cosmology).', hi: 'अग्नि जल में हैं — वडवाग्नि (हिंदू ब्रह्माण्ड की समुद्री अग्नि)।', sa: 'अग्नि जल में हैं — वडवाग्नि (हिंदू ब्रह्माण्ड की समुद्री अग्नि)।', mai: 'अग्नि जल में हैं — वडवाग्नि (हिंदू ब्रह्माण्ड की समुद्री अग्नि)।', mr: 'अग्नि जल में हैं — वडवाग्नि (हिंदू ब्रह्माण्ड की समुद्री अग्नि)।', ta: 'Agni is submerged in water — the Vadavagni (submarine fire of Hindu cosmology).', te: 'Agni is submerged in water — the Vadavagni (submarine fire of Hindu cosmology).', bn: 'Agni is submerged in water — the Vadavagni (submarine fire of Hindu cosmology).', kn: 'Agni is submerged in water — the Vadavagni (submarine fire of Hindu cosmology).', gu: 'Agni is submerged in water — the Vadavagni (submarine fire of Hindu cosmology).' },
                    ritualNote: { en: 'Rituals combining fire + water (abhishek after homa) uniquely powerful', hi: 'अग्नि + जल संयुक्त अनुष्ठान (होम बाद अभिषेक) विशेष शक्तिशाली', sa: 'अग्नि + जल संयुक्त अनुष्ठान (होम बाद अभिषेक) विशेष शक्तिशाली', mai: 'अग्नि + जल संयुक्त अनुष्ठान (होम बाद अभिषेक) विशेष शक्तिशाली', mr: 'अग्नि + जल संयुक्त अनुष्ठान (होम बाद अभिषेक) विशेष शक्तिशाली', ta: 'Rituals combining fire + water (abhishek after homa) uniquely powerful', te: 'Rituals combining fire + water (abhishek after homa) uniquely powerful', bn: 'Rituals combining fire + water (abhishek after homa) uniquely powerful', kn: 'Rituals combining fire + water (abhishek after homa) uniquely powerful', gu: 'Rituals combining fire + water (abhishek after homa) uniquely powerful' },
                  },
                };
              ...
              {panchang.chandraVaas && (() => {
                ...
                const chandraDesc: Record<string, { nColor: string; border: string; bg: string; desc: LocaleText; activities: LocaleText }> = {
                  "Brahma's Abode": {
                    ...
                    desc: { en: 'Moon faces East in Deva (celestial) abode. Divine energy flows freely — prayers answered with ease. Most auspicious pada for sacred activities.', hi: 'चंद्रमा पूर्व दिशा में देव निवास में हैं। दिव्य ऊर्जा स्वतंत्र प्रवाहित। प्रार्थनाएं सुनी जाती हैं — सर्वोत्तम शुभ पाद।', sa: 'चंद्रमा पूर्व दिशा में देव निवास में हैं। दिव्य ऊर्जा स्वतंत्र प्रवाहित। प्रार्थनाएं सुनी जाती हैं — सर्वोत्तम शुभ पाद।', mai: 'चंद्रमा पूर्व दिशा में देव निवास में हैं। दिव्य ऊर्जा स्वतंत्र प्रवाहित। प्रार्थनाएं सुनी जाती हैं — सर्वोत्तम शुभ पाद।', mr: 'चंद्रमा पूर्व दिशा में देव निवास में हैं। दिव्य ऊर्जा स्वतंत्र प्रवाहित। प्रार्थनाएं सुनी जाती हैं — सर्वोत्तम शुभ पाद।', ta: 'Moon faces East in Deva (celestial) abode. Divine energy flows freely — prayers answered with ease. Most auspicious pada for sacred activities.', te: 'Moon faces East in Deva (celestial) abode. Divine energy flows freely — prayers answered with ease. Most auspicious pada for sacred activities.', bn: 'Moon faces East in Deva (celestial) abode. Divine energy flows freely — prayers answered with ease. Most auspicious pada for sacred activities.', kn: 'Moon faces East in Deva (celestial) abode. Divine energy flows freely — prayers answered with ease. Most auspicious pada for sacred activities.', gu: 'Moon faces East in Deva (celestial) abode. Divine energy flows freely — prayers answered with ease. Most auspicious pada for sacred activities.' },
                    activities: { en: 'All auspicious work, puja, sacred ceremonies', hi: 'सभी शुभ कार्य, पूजा, पवित्र समारोह', sa: 'सभी शुभ कार्य, पूजा, पवित्र समारोह', mai: 'सभी शुभ कार्य, पूजा, पवित्र समारोह', mr: 'सभी शुभ कार्य, पूजा, पवित्र समारोह', ta: 'All auspicious work, puja, sacred ceremonies', te: 'All auspicious work, puja, sacred ceremonies', bn: 'All auspicious work, puja, sacred ceremonies', kn: 'All auspicious work, puja, sacred ceremonies', gu: 'All auspicious work, puja, sacred ceremonies' },
                  },
                  "Indra's Abode": {
                    nColor: 'text-blue-300', border: 'border-blue-500/25', bg: 'from-blue-500/5',
                    desc: { en: 'Moon faces South in Nara (human) abode — ordinary activity plane. Results are as expected, neither elevated nor hindered.', hi: 'चंद्रमा दक्षिण दिशा में मानव निवास में हैं। परिणाम सामान्य — न उन्नत, न बाधित।', sa: 'चंद्रमा दक्षिण दिशा में मानव निवास में हैं। परिणाम सामान्य — न उन्नत, न बाधित।', mai: 'चंद्रमा दक्षिण दिशा में मानव निवास में हैं। परिणाम सामान्य — न उन्नत, न बाधित।', mr: 'चंद्रमा दक्षिण दिशा में मानव निवास में हैं। परिणाम सामान्य — न उन्नत, न बाधित।', ta: 'Moon faces South in Nara (human) abode — ordinary activity plane. Results are as expected, neither elevated nor hindered.', te: 'Moon faces South in Nara (human) abode — ordinary activity plane. Results are as expected, neither elevated nor hindered.', bn: 'Moon faces South in Nara (human) abode — ordinary activity plane. Results are as expected, neither elevated nor hindered.', kn: 'Moon faces South in Nara (human) abode — ordinary activity plane. Results are as expected, neither elevated nor hindered.', gu: 'Moon faces South in Nara (human) abode — ordinary activity plane. Results are as expected, neither elevated nor hindered.' },
                    activities: { en: 'Daily work, business, social activities', hi: 'दैनिक कार्य, व्यापार, सामाजिक गतिविधियाँ', sa: 'दैनिक कार्य, व्यापार, सामाजिक गतिविधियाँ', mai: 'दैनिक कार्य, व्यापार, सामाजिक गतिविधियाँ', mr: 'दैनिक कार्य, व्यापार, सामाजिक गतिविधियाँ', ta: 'Daily work, business, social activities', te: 'Daily work, business, social activities', bn: 'Daily work, business, social activities', kn: 'Daily work, business, social activities', gu: 'Daily work, business, social activities' },
                  },
                  "Yama's Abode": {
                    nColor: 'text-amber-400', border: 'border-amber-500/25', bg: 'from-amber-500/5',
                    desc: { en: 'Moon faces West in Pashava (animal) abode — instinctual, reactive energy. Actions driven by impulse. Avoid important decisions.', hi: 'चंद्रमा पश्चिम दिशा में पशव निवास में हैं — सहज, प्रतिक्रियाशील ऊर्जा। महत्वपूर्ण निर्णयों से बचें।', sa: 'चंद्रमा पश्चिम दिशा में पशव निवास में हैं — सहज, प्रतिक्रियाशील ऊर्जा। महत्वपूर्ण निर्णयों से बचें।', mai: 'चंद्रमा पश्चिम दिशा में पशव निवास में हैं — सहज, प्रतिक्रियाशील ऊर्जा। महत्वपूर्ण निर्णयों से बचें।', mr: 'चंद्रमा पश्चिम दिशा में पशव निवास में हैं — सहज, प्रतिक्रियाशील ऊर्जा। महत्वपूर्ण निर्णयों से बचें।', ta: 'Moon faces West in Pashava (animal) abode — instinctual, reactive energy. Actions driven by impulse. Avoid important decisions.', te: 'Moon faces West in Pashava (animal) abode — instinctual, reactive energy. Actions driven by impulse. Avoid important decisions.', bn: 'Moon faces West in Pashava (animal) abode — instinctual, reactive energy. Actions driven by impulse. Avoid important decisions.', kn: 'Moon faces West in Pashava (animal) abode — instinctual, reactive energy. Actions driven by impulse. Avoid important decisions.', gu: 'Moon faces West in Pashava (animal) abode — instinctual, reactive energy. Actions driven by impulse. Avoid important decisions.' },
                    activities: { en: 'Physical labor, farming; avoid important decisions', hi: 'शारीरिक श्रम, खेती; महत्वपूर्ण निर्णयों से बचें', sa: 'शारीरिक श्रम, खेती; महत्वपूर्ण निर्णयों से बचें', mai: 'शारीरिक श्रम, खेती; महत्वपूर्ण निर्णयों से बचें', mr: 'शारीरिक श्रम, खेती; महत्वपूर्ण निर्णयों से बचें', ta: 'Physical labor, farming; avoid important decisions', te: 'Physical labor, farming; avoid important decisions', bn: 'Physical labor, farming; avoid important decisions', kn: 'Physical labor, farming; avoid important decisions', gu: 'Physical labor, farming; avoid important decisions' },
                  },
                  "Soma's Abode": {
                    nColor: 'text-red-400', border: 'border-red-500/25', bg: 'from-red-500/5',
                    desc: { en: 'Moon faces North in Rakshasa (demonic) abode — turbulent, obstructive energy. Activities face opposition or hidden obstacles.', hi: 'चंद्रमा उत्तर दिशा में राक्षस निवास में हैं — अशांत, अवरोधक ऊर्जा। विरोध या छिपी बाधाएं।', sa: 'चंद्रमा उत्तर दिशा में राक्षस निवास में हैं — अशांत, अवरोधक ऊर्जा। विरोध या छिपी बाधाएं।', mai: 'चंद्रमा उत्तर दिशा में राक्षस निवास में हैं — अशांत, अवरोधक ऊर्जा। विरोध या छिपी बाधाएं।', mr: 'चंद्रमा उत्तर दिशा में राक्षस निवास में हैं — अशांत, अवरोधक ऊर्जा। विरोध या छिपी बाधाएं।', ta: 'Moon faces North in Rakshasa (demonic) abode — turbulent, obstructive energy. Activities face opposition or hidden obstacles.', te: 'Moon faces North in Rakshasa (demonic) abode — turbulent, obstructive energy. Activities face opposition or hidden obstacles.', bn: 'Moon faces North in Rakshasa (demonic) abode — turbulent, obstructive energy. Activities face opposition or hidden obstacles.', kn: 'Moon faces North in Rakshasa (demonic) abode — turbulent, obstructive energy. Activities face opposition or hidden obstacles.', gu: 'Moon faces North in Rakshasa (demonic) abode — turbulent, obstructive energy. Activities face opposition or hidden obstacles.' },
                    activities: { en: 'Avoid sacred activities; protective rites permitted', hi: 'पवित्र गतिविधियों से बचें; सुरक्षात्मक अनुष्ठान संभव', sa: 'पवित्र गतिविधियों से बचें; सुरक्षात्मक अनुष्ठान संभव', mai: 'पवित्र गतिविधियों से बचें; सुरक्षात्मक अनुष्ठान संभव', mr: 'पवित्र गतिविधियों से बचें; सुरक्षात्मक अनुष्ठान संभव', ta: 'Avoid sacred activities; protective rites permitted', te: 'Avoid sacred activities; protective rites permitted', bn: 'Avoid sacred activities; protective rites permitted', kn: 'Avoid sacred activities; protective rites permitted', gu: 'Avoid sacred activities; protective rites permitted' },
                  },
                };
          ...
          <div className="my-14">
            <SectionHeading
              ...
              title={isTamil ? 'நாட்காட்டி & யுகம்' : locale === 'en' ? 'Calendars & Epoch' : 'पंचांग एवं युग'}
              subtitle={isTamil
                ? 'இந்து நாட்காட்டி முறை — சந்திர மாதங்கள், பருவங்கள், அண்ட சுழற்சிகள், ஆழ் காலத்தில் நமது நிலை.'
                : locale === 'en'
            ...
            <div className="mb-4">
              <h3 className="text-lg font-bold text-gold-light mb-4 flex items-center gap-2" style={headingFont}>
                <MasaIcon size={28} />
                <span>{isTamil ? 'இந்து நாட்காட்டி' : locale === 'en' ? 'Hindu Calendar' : 'हिन्दू पञ्चाङ्ग'}</span>
              </h3>
              <div className="rounded-2xl bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 overflow-hidden">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 divide-x divide-y divide-gold-primary/10">
                  {([
                    { label: isTamil ? 'விக்கிரம சம்வத்' : locale === 'en' ? 'Vikram Samvat' : 'विक्रम संवत्', value: panchang.vikramSamvat?.toString() || '—', iconKey: null },
                    { label: isTamil ? 'சக சம்வத்' : locale === 'en' ? 'Shaka Samvat' : 'शक संवत्', value: panchang.shakaSamvat?.toString() || '—', iconKey: null },
                    { label: t('samvatsara'), value: panchang.samvatsara[locale], iconKey: 'samvatsara' as const },
                    { label: isTamil ? `மாதம் (${masaSystem === 'purnimant' ? 'பூர்ணிமாந்தம்' : 'அமாந்தம்'})` : locale === 'en' ? `Masa (${masaSystem === 'purnimant' ? 'Purnimant' : 'Amant'})` : `मास (${masaSystem === 'purnimant' ? 'पूर्णिमान्त' : 'अमान्त'})`, value: masaSystem === 'purnimant' ? (panchang.purnimantMasa || panchang.masa)[locale] : (panchang.amantMasa || panchang.masa)[locale], iconKey: 'masa' as const },
                    { label: isTamil ? 'பக்ஷம்' : locale === 'en' ? 'Paksha' : 'पक्ष', value: panchang.tithi.paksha === 'shukla' ? (isTamil ? 'சுக்ல பக்ஷம்' : locale === 'en' ? 'Shukla Paksha' : 'शुक्ल पक्ष') : (isTamil ? 'கிருஷ்ண பக்ஷம்' : locale === 'en' ? 'Krishna Paksha' : 'कृष्ण पक्ष'), iconKey: null },
                    ...
                    { label: isTamil ? 'அயனாம்சம் (லாஹிரி)' : locale === 'en' ? 'Ayanamsha (Lahiri)' : 'अयनांश (लाहिरी)', value: panchang.ayanamsha ? `${panchang.ayanamsha.toFixed(4)}°` : '—', iconKey: null },
                    { label: isTamil ? 'பகல் நேரம்' : locale === 'en' ? 'Day Duration' : 'दिनमान', value: panchang.dinamana || '—', iconKey: null },
                    { label: isTamil ? 'இரவு நேரம்' : locale === 'en' ? 'Night Duration' : 'रात्रिमान', value: panchang.ratrimana || '—', iconKey: null },
                    { label: isTamil ? 'மத்தியான்னம்' : locale === 'en' ? 'Madhyahna' : 'मध्याह्न', value: panchang.madhyahna || '—', iconKey: null },
                ...
                <div className="mt-6 rounded-2xl bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 p-5">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-gold-light font-bold text-sm flex items-center gap-2" style={headingFont}>
                      <MasaIcon size={22} />
                      {isTamil ? `இந்து மாதங்கள் ${displayYear}` : locale === 'en' ? `Hindu Months ${displayYear}` : `हिन्दू मास ${displayYear}`}
                    </h4>
                    ...
                    <div className="inline-flex rounded-lg border border-gold-primary/15 overflow-hidden text-xs">
                      ...
                      >
                        {isTamil ? 'அமாந்தம்' : locale === 'en' ? 'Amant' : 'अमान्त'}
                      </button>
                      <button
                      ...
                      >
                        {isTamil ? 'பூர்ணிமாந்தம்' : locale === 'en' ? 'Purnimant' : 'पूर्णिमान्त'}
                      </button>
                    </div>
                  </div>
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-gold-primary/15">
                          ...
                          <th className="text-left py-2 px-2 text-gold-dark">{isTamil ? 'இந்து மாதம்' : locale === 'en' ? 'Hindu Month' : 'हिन्दू मास'}</th>
                          <th className="text-left py-2 px-2 text-gold-dark">{isTamil ? 'சமஸ்கிருதம்' : locale === 'en' ? 'Sanskrit' : 'संस्कृत'}</th>
                          <th className="text-left py-2 px-2 text-gold-dark">{isTamil ? 'தொடக்கம்' : locale === 'en' ? 'Start' : 'आरम्भ'}</th>
                          <th className="text-left py-2 px-2 text-gold-dark">{isTamil ? 'முடிவு' : locale === 'en' ? 'End' : 'समाप्ति'}</th>
                          <th className="text-left py-2 px-2 text-gold-dark">{isTamil ? 'ருது' : locale === 'en' ? 'Ritu' : 'ऋतु'}</th>
                        </tr>
                      </thead>
            ...
            <div className="mt-8">
              <h3 className="text-lg font-bold text-gold-light mb-4 flex items-center gap-2" style={headingFont}>
                ...
                <span>{isTamil ? 'யுகம் & அண்ட காலம்' : locale === 'en' ? 'Epoch & Cosmic Time' : 'युगीय काल'}</span>
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                  ...
                  <div className="text-gold-dark text-xs uppercase tracking-widest font-bold mb-1">
                    {isTamil ? 'கலி அஹர்கணம்' : locale === 'en' ? 'Kali Ahargana' : 'कलि अहर्गण'}
                  </div>
                  <div className="text-gold-light font-bold text-3xl font-mono mt-1">
                    {panchang.kaliAhargana?.toLocaleString() || '—'}
                  </div>
                  <div className="text-text-secondary text-xs mt-2">
                    {isTamil ? 'கலியுகம் தொடங்கியதிலிருந்து (பிப்ரவரி 18, 3102 கி.மு.) கடந்த நாட்கள்' : locale === 'en' ? 'Days elapsed since Kali Yuga began (Feb 18, 3102 BCE)' : 'कलियुग आरम्भ (18 फरवरी 3102 ईसापूर्व) से बीते दिन'}
                  </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }}
                  ...
                  <div className="text-amber-400 text-xs uppercase tracking-widest font-bold mb-1">
                    {isTamil ? 'கலியுக ஆண்டு' : locale === 'en' ? 'Kaliyuga Year' : 'कलियुग वर्ष'}
                  </div>
                  <div className="text-amber-300 font-bold text-3xl font-mono mt-1">
                    {panchang.kaliyugaYear?.toLocaleString() || '—'}
                  </div>
                  <div className="text-text-secondary text-xs mt-2">
                    {isTamil ? 'கலியுக சுழற்சியில் நடப்பு ஆண்டு (மொத்தம் 4,32,000 ஆண்டுகள்)' : locale === 'en' ? 'Current year in the Kali Yuga cycle (432,000 years total)' : 'कलियुग चक्र में वर्तमान वर्ष (कुल 4,32,000 वर्ष)'}
                  </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }}
                  ...
                  <div className="text-indigo-400 text-xs uppercase tracking-widest font-bold mb-1">
                    {isTamil ? 'ஜூலியன் நாள் எண்' : locale === 'en' ? 'Julian Day Number' : 'जूलियन दिन संख्या'}
                  </div>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {isCurrent && <span className="ml-2 px-2 py-0.5 bg-emerald-500/20 text-emerald-400 text-xs rounded-full font-bold animate-pulse">{isTamil ? 'இப்போது' : locale === 'en' ? 'NOW' : 'अभी'}</span>}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-5 my-14">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
                  ...
                  <div className="text-text-secondary text-xs mt-1">
                    ...
                    {panchang.moonSign?.pada ? ` · ${isTamil ? 'பாதம்' : locale === 'en' ? 'Pada' : 'पाद'} ${panchang.moonSign.pada}` : ''}
                  </div>
                </motion.div>
