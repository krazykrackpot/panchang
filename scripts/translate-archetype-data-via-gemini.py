#!/usr/bin/env python3
"""
Translate archetype-data (cosmic-blueprint engine: ARCHETYPES + YOGA_PSYCH_INSIGHTS + LAGNA_MODIFIERS) to 7 visible
regional locales via Gemini 2.5 Flash on Vertex AI.

Output: src/lib/constants/archetype-data-{locale}-overlay.json, keyed by
"archetype.<planetId>.<field>" / "yoga.<yogaId>" / "lagna.<rashiId>".
...
JOBS_FILE = Path("/tmp/ad-jobs.json")
...
def _write_overlay(locale: str, overlay: dict[str, str]) -> None:
    out_path = OUT_DIR / f"archetype-data-{locale}-overlay.json"
    merged: dict[str, str] = {}
    if out_path.exists():
        try:
