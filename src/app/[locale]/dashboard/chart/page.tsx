...
export default function ChartPage() {
  ...
  if (!user) {
    return <div className="max-w-2xl mx-auto px-4 py-20 text-center"><p className="text-text-secondary">{locale === 'en' ? 'Sign in to view your chart' : 'चार्ट देखने के लिए साइन इन करें'}</p></div>;
  }
  ...
  if (!kundali) {
    return <div className="max-w-2xl mx-auto px-4 py-20 text-center"><p className="text-text-secondary">{locale === 'en' ? 'Complete your profile to see your birth chart' : 'जन्म कुण्डली देखने के लिए प्रोफ़ाइल पूरा करें'}</p><a href={`/${locale}/settings`} className="text-gold-primary text-sm mt-2 inline-block">{locale === 'en' ? 'Go to Settings' : 'सेटिंग्स पर जाएँ'}</a></div>;
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-4xl font-bold mb-3" style={hf}><span className="text-gold-gradient">{locale === 'en' ? 'Your Birth Chart' : 'आपकी जन्म कुण्डली'}</span></h1>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-text-secondary border-b border-gold-primary/10 text-xs">
              <th className="text-left py-2 px-2">{locale === 'en' ? 'Planet' : 'ग्रह'}</th>
              <th className="text-left py-2 px-2">{locale === 'en' ? 'Sign' : 'राशि'}</th>
              <th className="text-left py-2 px-2">{locale === 'en' ? 'Degree' : 'अंश'}</th>
              <th className="text-left py-2 px-2">{locale === 'en' ? 'Nakshatra' : 'नक्षत्र'}</th>
              <th className="text-center py-2 px-2">{locale === 'en' ? 'House' : 'भाव'}</th>
      ...
      <div className="text-center mt-8">
        <a href={`/${locale}/kundali`} className="text-gold-primary hover:text-gold-light text-sm">
          {locale === 'en' ? 'Generate detailed analysis with all tabs →' : 'सभी टैब के साथ विस्तृत विश्लेषण →'}
        </a>
      </div>
    </div>
  );
