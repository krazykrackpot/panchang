// src/lib/kundali/health-diagnosis/__tests__/elements/longevity.test.ts
// B22 — classical badge, opt-in, requiresDisclaimer: true
// Note: Pinda Ayurdaya is stubbed (Phase E TODO) — score will be valid but not
// based on real Ayurdaya computation.
import { describe, it, expect } from 'vitest';
import { generateKundali } from '@/lib/ephem/kundali-calc';
import type { BirthData } from '@/types/kundali';
import { collectStrengthInputs } from '../../strength-inputs';
import { detectAllSignatures } from '../../signatures';
import { scoreLongevity } from '../../elements/longevity';
...
const result = scoreLongevity(kundali, strength, signatures, 'en');
...
describe('scoreLongevity', () => {
  it('returns a NatalElement with score in [0,100], valid rating, and at least 4 factors', () => {
    ...
    // longevity requires disclaimer
    expect(result.requiresDisclaimer).toBe(true);
  });
  it('returns correct metadata matching ELEMENT_CATALOG[longevity]', () => {
    const meta = ELEMENT_CATALOG['longevity'];
    expect(result.id).toBe('longevity');
