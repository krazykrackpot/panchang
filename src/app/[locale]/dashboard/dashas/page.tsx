...
export default function DashasPage() {
  ...
  if (!user) {
    return <div className="max-w-2xl mx-auto px-4 py-20 text-center"><p className="text-text-secondary">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Sign in to view your dashas' : 'दशा देखने के लिए साइन इन करें'}</p></div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-4xl font-bold mb-3" style={hf}><span className="text-gold-gradient">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Dasha Timeline' : 'दशा समयरेखा'}</span></h1>
      </motion.div>
      ...
      {currentMaha && (
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-2xl p-6 mb-8">
          <p className="text-gold-dark text-xs uppercase tracking-wider font-bold mb-4">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Current Period' : 'वर्तमान काल'}</p>

          {/* Mahadasha */}
          <div className="flex items-center gap-4 mb-4">
            <GrahaIconById id={PLANET_ID[currentMaha.planet] ?? 0} size={40} />
            <div className="flex-1">
              <p className="text-gold-light font-bold text-lg" style={hf}>{currentMaha.planetName[lk as keyof typeof currentMaha.planetName]} {(locale !== 'hi' && String(locale) !== 'sa') ? 'Mahadasha' : 'महादशा'}</p>
              <p className="text-text-secondary text-xs font-mono">{currentMaha.startDate} — {currentMaha.endDate} ({yearsDiff(currentMaha.startDate, currentMaha.endDate)} {(locale !== 'hi' && String(locale) !== 'sa') ? 'years' : 'वर्ष'})</p>
              ...
              <p className="text-text-secondary/70 text-xs mt-1">{progressPercent(currentMaha.startDate, currentMaha.endDate)}% {(locale !== 'hi' && String(locale) !== 'sa') ? 'complete' : 'पूर्ण'}</p>
            </div>
          </div>
          ...
          {currentAntar && (
            <div className="flex items-center gap-4 ml-8 pl-4 border-l-2 border-gold-primary/20">
              ...
              <div className="flex-1">
                <p className="text-gold-light font-semibold text-sm" style={bf}>{currentAntar.planetName[lk as keyof typeof currentAntar.planetName]} {(locale !== 'hi' && String(locale) !== 'sa') ? 'Antardasha' : 'अंतर्दशा'}</p>
      ...
      {/* Full Timeline */}
      <h2 className="text-xl font-bold text-gold-gradient mb-4" style={hf}>{(locale !== 'hi' && String(locale) !== 'sa') ? 'Full Timeline' : 'पूर्ण समयरेखा'}</h2>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {isCurrent && <span className="ml-2 text-xs px-2 py-0.5 bg-gold-primary/20 text-gold-primary rounded-full font-bold">{(locale !== 'hi' && String(locale) !== 'sa') ? 'NOW' : 'अभी'}</span>}
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                    ...
                    {d.subPeriods.map((s, j) => {
                      ...
                      return (
                        <div key={j} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs ${isCurrentAntar ? 'bg-gold-primary/10 border border-gold-primary/20' : ''}`}>
                          ...
                          {isCurrentAntar && <span className="text-gold-primary text-xs font-bold">{(locale !== 'hi' && String(locale) !== 'sa') ? 'NOW' : 'अभी'}</span>}
                        </div>
                      );
                    })}
                  </motion.div>
