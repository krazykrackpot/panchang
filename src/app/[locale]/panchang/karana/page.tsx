...
function AngularSeparationDiagram({ locale }: { locale: Locale }) {
  return (
    ...
    >
      ...
      >
        {locale === 'en' ? '6° = 1 Karana = Half Tithi' : '6° = 1 करण = अर्ध तिथि'}
      </motion.text>
      ...
      >
        {locale === 'en' ? '12° = 1 Tithi = 2 Karanas' : '12° = 1 तिथि = 2 करण'}
      </motion.text>
      ...
      >
        ...
        <text x="532" y="142" fill="rgba(74,222,128,0.5)" fontSize="7" textAnchor="middle">
          {locale === 'en' ? '60 Karanas / month' : '60 करण / मास'}
        </text>
      </motion.g>
    </motion.svg>
  );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-6 mb-6">
        ...
        <div>
          <h1 className="text-4xl sm:text-5xl font-bold mb-4" style={headingFont}>
            <span className="text-gold-gradient">{locale === 'en' ? 'Karana' : locale === 'hi' ? 'करण' : 'करणम्'}</span>
          </h1>
          <p className="text-text-secondary text-lg" style={{ fontFamily: 'var(--font-heading)' }}>
            {locale === 'en' ? 'The Half-Tithi — 11 Building Blocks of Lunar Time' : locale === 'hi' ? 'अर्ध-तिथि — चान्द्र काल के 11 खण्ड' : 'अर्धतिथिः — चान्द्रकालस्य एकादश खण्डाः'}
          </p>
        </div>
      </motion.div>
          ...
          <div className="mt-8 prose prose-invert max-w-none text-text-secondary">
            ...
            >
              <p className="text-gold-light font-mono text-sm">
                {locale === 'en' ? 'Formula:' : 'सूत्र:'} Karana_index = floor((Moon_long - Sun_long) / 6°)
              </p>
              <p className="text-gold-light/70 font-mono text-xs mt-1">
                {locale === 'en' ? '60 Karanas = 7 Chara (cycle 8x) + 4 Sthira (once each)' : '60 करण = 7 चर (8 बार) + 4 स्थिर (एक बार)'}
              </p>
            </motion.div>
          </div>
        </div>
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full ${badgeClasses}`}>
                      {isSthira
                        ? (locale === 'en' ? 'Sthira (Fixed)' : 'स्थिर')
                        : karana.name.en === 'Vishti'
                        ? (locale === 'en' ? 'Vishti / Bhadra' : 'विष्टि / भद्रा')
                        : (locale === 'en' ? 'Chara (Movable)' : 'चर')}
                    </span>
                  </div>
                  <div className="text-text-secondary text-sm space-y-1">
                    <p>
                      <span className="text-gold-dark">{locale === 'en' ? 'Position:' : 'स्थान:'}</span>{' '}
                      {selectedKarana + 1} / 60
                    </p>
                    <p>
                      <span className="text-gold-dark">{locale === 'en' ? 'Angular Span:' : 'कोणीय विस्तार:'}</span>{' '}
                      {(selectedKarana * 6)}° — {((selectedKarana + 1) * 6)}°
                    </p>
                    <p>
                      <span className="text-gold-dark">{locale === 'en' ? 'Within Tithi:' : 'तिथि में:'}</span>{' '}
                      {locale === 'en'
                    ...
                    <p>
                      <span className="text-gold-dark">{locale === 'en' ? 'Karana #:' : 'करण #:'}</span>{' '}
      ...
      <section className="my-12">
        ...
        <h3 className="text-xl text-gold-light mb-4" style={headingFont}>
          {locale === 'en' ? 'Chara Karanas (Movable — cycle 8 times)' : locale === 'hi' ? 'चर करण (गतिशील — 8 बार चक्र)' : 'चरकरणानि (चलानि — अष्टवारं चक्रम्)'}
        </h3>
          ...
          {chara.map((karana, i) => (
            ...
            >
              ...
              <div className={`text-xs mt-1 ${karana.name.en === 'Vishti' ? 'text-red-400' : 'text-emerald-400'}`}>
                {karana.name.en === 'Vishti'
                  ? (locale === 'en' ? 'Vishti / Bhadra' : 'विष्टि / भद्रा')
                  : (locale === 'en' ? 'Chara (Movable)' : locale === 'hi' ? 'चर (गतिशील)' : 'चरम्')}
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <h3 className="text-xl text-red-400/80 mb-4" style={headingFont}>
          {locale === 'en' ? 'Sthira Karanas (Fixed — appear once each)' : locale === 'hi' ? 'स्थिर करण (अचल — प्रत्येक एक बार)' : 'स्थिरकरणानि (अचलानि — एकवारम्)'}
        </h3>
