  ...
  return (
    <div>
      ...
      <motion.div
        ...
        className="relative overflow-hidden rounded-3xl border border-amber-400/20 bg-gradient-to-br from-amber-950/90 via-yellow-950/70 to-orange-950/90 p-8 sm:p-10 mb-10"
      >
        ...
        <div className="relative z-10">
          <Link href="/learn" className="inline-flex items-center gap-1.5 text-amber-300/70 hover:text-amber-200 text-xs uppercase tracking-wider mb-6 transition-colors">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-amber-300 text-xs uppercase tracking-widest font-bold">{l.badge}</span>
          </div>
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl bg-amber-500 text-black font-bold text-sm hover:bg-amber-400 transition-colors"
            style={hf}
          >
            {l.startHere}
      ...
      <div className="space-y-8">
        {SECTIONS.map((section, si) => (
          <motion.div
            ...
            className="rounded-2xl border border-amber-500/15 bg-amber-950/20 overflow-hidden"
          >
            {/* Section header */}
            <div className="px-6 py-4 border-b border-amber-500/10 flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-amber-300/50 text-xs uppercase tracking-wider" style={bf}>{section.subtitle[locale]}</span>
              </div>
              {section.modules.length > 0 && (
                <span className="ml-auto text-amber-400/40 text-xs font-mono">{section.modules.length} {l.modules}</span>
              )}
              {section.modules.length === 0 && (
                <span className="ml-auto text-amber-400/30 text-xs italic">{l.refsOnly}</span>
              )}
            </div>
              ...
              <div className="divide-y divide-amber-500/8">
                {section.modules.map((mod, mi) => (
                  ...
                  >
                    ...
                    <ChevronRight className="w-4 h-4 text-text-tertiary group-hover:text-amber-300 transition-colors shrink-0" />
                  </Link>
                ))}
              </div>
            )}
