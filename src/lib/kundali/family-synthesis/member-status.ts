...
export function computeMemberStatus(params: MemberStatusParams): MemberStatus {
  ...
  return {
    ...
    birthYear: kundali.birthData?.date ? new Date(kundali.birthData.date).getFullYear() : 0,
    ...
    birthPaksha: (() => { const sl = kundali.planets[0]?.longitude ?? 0; const ml = kundali.planets[1]?.longitude ?? 0; const t = Math.floor(((ml - sl + 360) % 360) / 12) + 1; return t <= 15 ? 'Shukla' : 'Krishna'; })(),
    currentDasha: {
