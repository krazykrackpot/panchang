  ...
  return (
    <div>
      ...
      <LessonSection title={L.exampleTitle[locale]} variant="highlight">
        ...
        <div className="mt-4 p-5 rounded-xl bg-gold-primary/5 border border-gold-primary/20">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-gold-primary text-xs uppercase tracking-widest mb-1">{locale === 'en' || String(locale) === 'ta' ? 'Date' : 'तिथि'}</div>
              <div className="text-gold-light font-bold text-lg">15 Aug 1995</div>
            </div>
            <div>
              <div className="text-gold-primary text-xs uppercase tracking-widest mb-1">{locale === 'en' || String(locale) === 'ta' ? 'Time' : 'समय'}</div>
              <div className="text-gold-light font-bold text-lg">10:30 AM IST</div>
            </div>
            <div>
              <div className="text-gold-primary text-xs uppercase tracking-widest mb-1">{locale === 'en' || String(locale) === 'ta' ? 'Place' : 'स्थान'}</div>
              <div className="text-gold-light font-bold text-lg">{locale === 'en' || String(locale) === 'ta' ? 'New Delhi' : 'नई दिल्ली'}</div>
            </div>
          </div>
      ...
      <LessonSection number={5} title={L.s5Title[locale]}>
        ...
        <div className="mt-3 text-right">
          <Link href="/learn/calculations" className="text-gold-primary hover:text-gold-light text-xs transition-colors">
            {locale === 'en' || String(locale) === 'ta' ? 'See full mathematical derivation' : 'पूर्ण गणितीय व्युत्पत्ति देखें'} →
          </Link>
        </div>
      </LessonSection>
      ...
      <LessonSection number={6} title={L.s6Title[locale]}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gold-primary/20">
                <th className="text-left py-2 text-gold-primary font-semibold text-xs">{locale === 'en' || String(locale) === 'ta' ? 'Graha' : 'ग्रह'}</th>
                <th className="text-left py-2 text-gold-primary font-semibold text-xs">{locale === 'en' || String(locale) === 'ta' ? 'Sidereal°' : 'नाक्ष°'}</th>
                <th className="text-left py-2 text-gold-primary font-semibold text-xs">{locale === 'en' || String(locale) === 'ta' ? 'Rashi' : 'राशि'}</th>
                <th className="text-left py-2 text-gold-primary font-semibold text-xs hidden sm:table-cell">{locale === 'en' || String(locale) === 'ta' ? 'Nakshatra' : 'नक्षत्र'}</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div className="mt-3 text-right">
          <Link href="/learn/grahas" className="text-gold-primary hover:text-gold-light text-xs transition-colors">
            {locale === 'en' || String(locale) === 'ta' ? 'Learn about all 9 Grahas' : 'सभी 9 ग्रहों के बारे में जानें'} →
          </Link>
        </div>
      </LessonSection>
      ...
      <LessonSection number={7} title={L.s7Title[locale]}>
        ...
        <div className="mt-3 text-right">
          <Link href="/learn/bhavas" className="text-gold-primary hover:text-gold-light text-xs transition-colors">
            {locale === 'en' || String(locale) === 'ta' ? 'Deep dive into all 12 houses' : 'सभी 12 भावों का विस्तृत अध्ययन'} →
          </Link>
        </div>
      </LessonSection>
      ...
      >
        ...
        <div className="mt-4 p-4 bg-bg-primary/50 rounded-lg border border-gold-primary/10">
          ...
          <div className="space-y-1 text-gold-light/70 font-mono text-xs">
            <p>{locale === 'en' || String(locale) === 'ta' ? '- Top diamond (House 1) = Lagna → Tula (Libra) with Rahu' : '- शीर्ष हीरा (भाव 1) = लग्न → तुला, राहु सहित'}</p>
            <p>{locale === 'en' || String(locale) === 'ta' ? '- Houses run counter-clockwise from the top' : '- भाव शीर्ष से वामावर्त चलते हैं'}</p>
            <p>{locale === 'en' || String(locale) === 'ta' ? '- Rashi names label each house; planets shown at center' : '- राशि नाम प्रत्येक भाव में; ग्रह केन्द्र में दिखाए गए'}</p>
            <p>{locale === 'en' || String(locale) === 'ta' ? '- Sun in 10th (career) = strong public presence' : '- सूर्य 10वें भाव (कर्म) में = शक्तिशाली सार्वजनिक उपस्थिति'}</p>
            <p>{locale === 'en' || String(locale) === 'ta' ? '- Moon in 3rd (communication) = expressive mind' : '- चन्द्र 3रे भाव (संवाद) में = अभिव्यक्तिशील मन'}</p>
          </div>
        </div>
      </LessonSection>
      ...
      <LessonSection number={9} title={L.s9Title[locale]}>
        ...
        <div className="mt-4 p-4 bg-bg-primary/50 rounded-lg border border-gold-primary/10">
          <p className="text-gold-light font-mono text-sm mb-2">{locale === 'en' || String(locale) === 'ta' ? 'Dignity Hierarchy:' : 'गरिमा क्रम:'}</p>
      ...
      <LessonSection number={10} title={L.s10Title[locale]}>
        ...
        <div className="mt-4 p-4 bg-bg-primary/50 rounded-lg border border-gold-primary/10">
          <p className="text-gold-light font-mono text-sm mb-2">{locale === 'en' || String(locale) === 'ta' ? 'In our example:' : 'हमारे उदाहरण में:'}</p>
          <div className="space-y-1 text-gold-light/70 font-mono text-xs">
            <p>{locale === 'en' || String(locale) === 'ta' ? 'Jupiter in 2nd aspects 8th → protects longevity house' : 'गुरु 2रे भाव में → 8वें भाव पर दृष्टि → आयु भाव की रक्षा'}</p>
            <p>{locale === 'en' || String(locale) === 'ta' ? 'Saturn in 5th aspects 7th, 11th, 2nd → discipline in marriage, gains, speech' : 'शनि 5वें भाव में → 7, 11, 2 पर दृष्टि → विवाह, लाभ, वाणी में अनुशासन'}</p>
            <p>{locale === 'en' || String(locale) === 'ta' ? 'Sun in 10th aspects 4th → career impacts home life' : 'सूर्य 10वें में → 4वें पर दृष्टि → कर्म गृह जीवन प्रभावित करता है'}</p>
          </div>
        </div>
      </LessonSection>
      ...
      <LessonSection number={11} title={L.s11Title[locale]}>
        ...
        <div className="mt-4 p-4 bg-bg-primary/50 rounded-lg border border-gold-primary/10">
          ...
          <div className="space-y-1">
            <p className="text-gold-light/80 font-mono text-xs">{locale === 'en' || String(locale) === 'ta' ? 'Moon at 253.0° sidereal → Purva Ashadha Nakshatra' : 'चन्द्र 253.0° नाक्षत्रिक → पूर्वाषाढ़ा नक्षत्र'}</p>
            <p className="text-gold-light/80 font-mono text-xs">{locale === 'en' || String(locale) === 'ta' ? 'P. Ashadha lord = Venus → born in Venus Maha Dasha' : 'पूर्वाषाढ़ा स्वामी = शुक्र → शुक्र महादशा में जन्म'}</p>
            <p className="text-gold-light/80 font-mono text-xs">{locale === 'en' || String(locale) === 'ta' ? 'Venus Maha Dasha = 20 years total' : 'शुक्र महादशा = कुल 20 वर्ष'}</p>
            <p className="text-gold-light/80 font-mono text-xs mt-2">{locale === 'en' || String(locale) === 'ta' ? 'Moon progress through P. Ashadha:' : 'पूर्वाषाढ़ा में चन्द्र की प्रगति:'}</p>
            <p className="text-gold-light/80 font-mono text-xs">{locale === 'en' || String(locale) === 'ta' ? 'P. Ashadha span: 253°20\' to 266°40\' (13°20\')' : 'पूर्वाषाढ़ा: 253°20\' से 266°40\' (13°20\')'}</p>
            <p className="text-gold-light/80 font-mono text-xs">{locale === 'en' || String(locale) === 'ta' ? 'Moon at 253.0° → near the start → ~19.6 years of Venus remain' : 'चन्द्र 253.0° → प्रारम्भ के निकट → शुक्र के ~19.6 वर्ष शेष'}</p>
            <p className="text-gold-light/60 font-mono text-xs mt-2">{locale === 'en' || String(locale) === 'ta' ? 'After Venus: Sun (6y) → Moon (10y) → Mars (7y) → ...' : 'शुक्र के बाद: सूर्य (6 वर्ष) → चन्द्र (10 वर्ष) → मंगल (7 वर्ष) → ...'}</p>
          </div>
        </div>
        <div className="mt-3 text-right">
          <Link href="/learn/dashas" className="text-gold-primary hover:text-gold-light text-xs transition-colors">
            {locale === 'en' || String(locale) === 'ta' ? 'Complete Dasha system explained' : 'सम्पूर्ण दशा प्रणाली की व्याख्या'} →
          </Link>
        </div>
      </LessonSection>
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="rounded-lg p-4 border border-emerald-400/20 bg-emerald-400/5">
            ...
            <ul className="space-y-1 text-text-secondary text-xs">
              <li>{locale === 'en' || String(locale) === 'ta' ? 'Budhaditya Yoga — Mercury + Sun in same house (11th)' : 'बुधादित्य योग — बुध + सूर्य (यहाँ 10-11 में)'}</li>
              <li>{locale === 'en' || String(locale) === 'ta' ? 'Gajakesari Yoga — Jupiter in Kendra from Moon' : 'गजकेसरी योग — चन्द्र से केन्द्र में गुरु'}</li>
              <li>{locale === 'en' || String(locale) === 'ta' ? 'Dhana Yoga — lord of 2nd in good placement' : 'धन योग — 2 भाव का स्वामी शुभ स्थान में'}</li>
            </ul>
          </div>
          <div className="rounded-lg p-4 border border-red-400/20 bg-red-400/5">
            <h4 className="text-red-400 font-bold text-sm mb-2">{locale === 'en' || String(locale) === 'ta' ? 'Doshas (Afflictions)' : 'दोष (कठिनाइयाँ)'}</h4>
            <ul className="space-y-1 text-text-secondary text-xs">
              <li>{locale === 'en' || String(locale) === 'ta' ? 'Mangal Dosha — Mars in 1, 2, 4, 7, 8, or 12 from Lagna' : 'मंगल दोष — लग्न से 1, 2, 4, 7, 8, 12 में मंगल'}</li>
              <li>{locale === 'en' || String(locale) === 'ta' ? 'Kaal Sarpa — all planets between Rahu-Ketu axis' : 'काल सर्प — राहु-केतु अक्ष के बीच सभी ग्रह'}</li>
              <li>{locale === 'en' || String(locale) === 'ta' ? 'Pitru Dosha — Sun afflicted by malefics' : 'पितृ दोष — सूर्य पाप ग्रहों से पीड़ित'}</li>
            </ul>
          </div>
        </div>
        <div className="mt-4 p-4 bg-bg-primary/50 rounded-lg border border-gold-primary/10">
          <p className="text-gold-light font-mono text-sm mb-2">{locale === 'en' || String(locale) === 'ta' ? 'In our example:' : 'हमारे उदाहरण में:'}</p>
          <div className="space-y-1 text-gold-light/70 font-mono text-xs">
            <p>{locale === 'en' || String(locale) === 'ta' ? 'Mars in 12th from Lagna → Mangal Dosha is present' : 'लग्न से 12वें भाव में मंगल → मंगल दोष उपस्थित'}</p>
            <p>{locale === 'en' || String(locale) === 'ta' ? 'Mercury + Venus in Leo (11th) → Dhana Yoga (gains house)' : 'बुध + शुक्र सिंह (11वाँ) में → धन योग (लाभ भाव)'}</p>
            <p>{locale === 'en' || String(locale) === 'ta' ? 'Saturn in own sign Aquarius → strong 5th house (intelligence)' : 'शनि स्वराशि कुम्भ में → शक्तिशाली 5वाँ भाव (बुद्धि)'}</p>
          </div>
        </div>
      </LessonSection>
          ...
          {[
            ...
            <span key={step.en} className="flex items-center gap-2">
              <span className="px-3 py-1.5 rounded-full text-xs font-semibold bg-gold-primary/10 border border-gold-primary/20 text-gold-light">
                {locale === 'en' || String(locale) === 'ta' ? step.en : step.hi}
              </span>
