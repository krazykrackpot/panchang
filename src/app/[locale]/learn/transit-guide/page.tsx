      ...
      <AnimatePresence mode="wait">
        {houseData && (
          <motion.div key={selected} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}>
            <LessonSection number={2} title={`${t(activeTab.label)} — ${locale === 'hi' ? '12 भावों में गोचर' : locale === 'sa' ? '12 भावेषु गोचरः' : 'Transit Through 12 Houses'}`}>
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-bold" style={{ color: activeTab.color }}>
                        {locale === 'hi' ? `${h.house}वाँ भाव` : locale === 'sa' ? `${h.house} भावः` : `House ${h.house}`}
                      </span>
        ...
        {axisData && (
          <motion.div key="rahu_ketu" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}>
            <LessonSection number={2} title={`${t(activeTab.label)} — ${locale === 'hi' ? 'अक्ष प्रभाव' : locale === 'sa' ? 'अक्षप्रभावाः' : 'Axis Effects'}`}>
              <p className="mb-4 text-sm">
                {locale === 'hi'
                  ? 'राहु-केतु सदैव 180° विपरीत होते हैं। प्रभाव अक्ष (axis) जोड़ियों में पढ़ा जाता है।'
                  : locale === 'sa'
                  ? 'राहुकेतू सदा 180° विपरीतौ। प्रभावः अक्षयुग्मेषु पठ्यते।'
                  : 'Rahu and Ketu are always 180 degrees apart. Their effects are read as axis pairs.'}
              </p>
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      <span className="text-purple-300 font-bold text-sm">
                        {locale === 'hi' ? `राहु ${a.rahu}वें / केतु ${a.ketu}वें` : `Rahu ${a.rahu}th / Ketu ${a.ketu}th`}
                      </span>
                    </div>
