...
describe('sitemap URL budget', () => {
  // Generate once and share — buildSitemapEntries walks every route ×
  // locale and produces ~11K entries; calling it per test would dominate
  // the suite. The sitemap is served via app/sitemap.xml/route.ts which
  // gzips the response — the byte ceiling here is the RAW XML size, not
  // the on-wire size. Compressed payload is ~25× smaller, so the 45 MB
  // raw ceiling maps to ~1.8 MB on the wire (well under GSC's tolerance).
