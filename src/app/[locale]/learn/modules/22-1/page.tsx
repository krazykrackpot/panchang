...
function Page4() {
  ...
  return (
    <div className="space-y-6">
      <section>
        <h3 className="text-gold-light font-bold text-lg mb-3" style={{ fontFamily: 'var(--font-heading)' }}>
          {tl({ en: 'The Reverse Conversion: JD Back to Calendar Date', hi: 'उलटा रूपान्तरण: JD से कैलेण्डर तिथि', sa: 'उलटा रूपान्तरण: JD से कैलेण्डर तिथि' }, locale)}
        </h3>
        <p className="text-text-secondary text-sm leading-relaxed mb-3">{isHi ? <>जूलियन दिवस से कैलेण्डर तिथि में वापस जाना भी आवश्यक है  –  हम गणना JD में करते हैं किन्तु परिणाम मानव-पठनीय तिथियों में प्रदर्शित करते हैं। उलटा एल्गोरिदम: Z = floor(JD + 0.5), A = Z (यदि Z &lt; 2299161) अन्यथा alpha = floor((Z - 1867216.25) / 36524.25), A = Z + 1 + alpha - floor(alpha / 4)। फिर B = A + 1524, C = floor((B - 122.1) / 365.25), D = floor(365.25 × C), E = floor((B - D) / 30.6001)। दिन = B - D - floor(30.6001 × E), मास = E &lt; 14 ? E - 1 : E - 13, वर्ष = मास &gt; 2 ? C - 4716 : C - 4715।</> : <>Converting from Julian Day back to a calendar date is equally necessary  –  we compute in JD but display results as human-readable dates. The reverse algorithm: Z = floor(JD + 0.5), A = Z (if Z &lt; 2299161) else alpha = floor((Z - 1867216.25) / 36524.25), A = Z + 1 + alpha - floor(alpha / 4). Then B = A + 1524, C = floor((B - 122.1) / 365.25), D = floor(365.25 x C), E = floor((B - D) / 30.6001). Day = B - D - floor(30.6001 x E), Month = E &lt; 14 ? E - 1 : E - 13, Year = Month &gt; 2 ? C - 4716 : C - 4715.</>}</p>
        <p className="text-text-secondary text-sm leading-relaxed mb-3">{isHi ? <>यह उलटा रूपान्तरण (jdToDate) हमारे कोड में एक गम्भीर बग का स्रोत रहा है। मूल कार्यान्वयन ने new Date(year, month-1, day) प्रयोग किया  –  जो सर्वर के स्थानीय समयक्षेत्र में व्याख्या करता है। Vercel पर (UTC), यह सही है। किन्तु UTC+2 मशीन पर, JD मध्याह्न UT 24 अप्रैल का Date 23 अप्रैल 22:00 UTC दिखा सकता है। सही पैटर्न: सदैव Date.UTC(year, month-1, day) प्रयोग करें। यह बग हमारे ऑडिट में पकड़ा गया और कुण्डली तिथि गणनाओं में त्रुटि का कारण था।</> : <>This reverse conversion (jdToDate) has been a source of a critical bug in our code. The original implementation used new Date(year, month-1, day)  –  which interprets in the server&apos;s local timezone. On Vercel (UTC), this is fine. But on a UTC+2 machine, JD noon UT on April 24 could produce a Date showing April 23 22:00 UTC. The correct pattern: always use Date.UTC(year, month-1, day). This bug was caught in our audit and was causing errors in kundali date calculations.</>}</p>
      </section>

      <section className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-5 border border-emerald-500/15">
        <h4 className="text-emerald-400 text-xs uppercase tracking-widest font-bold mb-3">{tl({ en: 'Practical Application: Date Arithmetic in Jyotish', hi: 'व्यावहारिक अनुप्रयोग: ज्योतिष में तिथि गणित', sa: 'व्यावहारिक अनुप्रयोग: ज्योतिष में तिथि गणित' }, locale)}</h4>
        <p className="text-text-secondary text-xs leading-relaxed mb-2">{isHi ? <>JD तिथि गणित को तुच्छ बनाता है। विंशोत्तरी दशा में: &quot;सूर्य महादशा 6 वर्ष&quot; = 6 × 365.25 = 2191.5 दिन। यदि वर्तमान दशा JD 2461000.0 पर आरम्भ हुई, अगली JD 2463191.5 पर। jdToDate(2463191.5) = सटीक कैलेण्डर तिथि। कोई मास-लम्बाई, अधिवर्ष, या कैलेण्डर संक्रमण की चिन्ता नहीं।</> : <>JD makes date arithmetic trivial. In Vimshottari Dasha: &quot;Sun Mahadasha 6 years&quot; = 6 x 365.25 = 2191.5 days. If the current dasha started at JD 2461000.0, the next one starts at JD 2463191.5. jdToDate(2463191.5) = exact calendar date. No worrying about month lengths, leap years, or calendar transitions.</>}</p>
        <p className="text-text-secondary text-xs leading-relaxed mb-2">{isHi ? <>तिथि सीमा खोजने में: तिथि सीमा = चन्द्र-सूर्य कोण 12° का गुणज। JD = 2461132.0 पर कोण = 108° (शुक्ल नवमी) मान लें। अगली सीमा 120° (शुक्ल दशमी) पर है। चन्द्रमा सूर्य से ~12°/दिन दूर जाता है, अतः सीमा ~1 दिन बाद = JD 2461133.0 के आसपास। इसे बाइनरी सर्च से ~0.001 JD (1.4 मिनट) सटीकता तक शोधित करें।</> : <>For finding tithi boundaries: a tithi boundary occurs when the Moon-Sun angle is a multiple of 12°. Suppose at JD = 2461132.0 the angle is 108° (Shukla Navami). The next boundary is at 120° (Shukla Dashami). The Moon separates from the Sun at ~12°/day, so the boundary is ~1 day later = around JD 2461133.0. Refine with binary search to ~0.001 JD (1.4 minutes) precision.</>}</p>
        <p className="text-text-secondary text-xs leading-relaxed">{isHi ? <>सप्ताह का दिन (वार) निर्धारित करने में: floor(JD + 1.5) mod 7 वार देता है, जहाँ 0 = रविवार। JD 2461132.0 के लिए: floor(2461132.0 + 1.5) = 2461133, 2461133 mod 7 = 2461133 - 351590 × 7 = 2461133 - 2461130 = 3 = बुधवार। Date.getUTCDay() से तत्काल सत्यापन सम्भव, क्योंकि दोनों समान 0=रविवार परम्परा अपनाते हैं।</> : <>For determining the weekday (vara): floor(JD + 1.5) mod 7 gives the vara, where 0 = Sunday. For JD 2461132.0: floor(2461132.0 + 1.5) = 2461133, 2461133 mod 7 = 2461133 - 351590 x 7 = 2461133 - 2461130 = 3 = Wednesday. Instant verification is possible with Date.getUTCDay(), since both use the same 0=Sunday convention.</>}</p>
      </section>
      ...
      <section className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-5 border border-red-500/15">
        ...
        <p className="text-text-secondary text-xs leading-relaxed mb-2">{isHi ? <><span className="text-gold-light font-medium">भ्रान्ति:</span> &quot;Modified Julian Day (MJD) अधिक सटीक है।&quot; MJD = JD - 2400000.5। यह संख्या को छोटा बनाता है (फ़्लोटिंग-पॉइंट अनुकूल) और मध्यरात्रि से आरम्भ होता है (प्रोग्रामर-अनुकूल), किन्तु इसकी सटीकता समान है। हम JD प्रयोग करते हैं क्योंकि मीयस के सभी सूत्र JD पर आधारित हैं।</> : <><span className="text-gold-light font-medium">Misconception:</span> &quot;Modified Julian Day (MJD) is more accurate.&quot; MJD = JD - 2400000.5. It makes the number smaller (floating-point friendly) and starts at midnight (programmer-friendly), but its accuracy is identical. We use JD because all of Meeus&apos;s formulas are based on JD.</>}</p>
        <p className="text-text-secondary text-xs leading-relaxed">{isHi ? <><span className="text-gold-light font-medium">भ्रान्ति:</span> &quot;JavaScript की Date सभी ऐतिहासिक तिथियाँ सही संभालती है।&quot; JavaScript का Date ऑब्जेक्ट प्रोलेप्टिक ग्रेगोरियन कैलेण्डर प्रयोग करता है  –  यह ग्रेगोरियन नियम 1582 से पहले भी लागू करता है। ऐतिहासिक तिथियों (जैसे प्राचीन ग्रहण सत्यापन) के लिए JD एकमात्र विश्वसनीय विधि है, क्योंकि JD कैलेण्डर-अज्ञेयवादी है।</> : <><span className="text-gold-light font-medium">Misconception:</span> &quot;JavaScript&apos;s Date handles all historical dates correctly.&quot; JavaScript&apos;s Date object uses the proleptic Gregorian calendar  –  it applies Gregorian rules even before 1582. For historical dates (like ancient eclipse verification), JD is the only reliable method because JD is calendar-agnostic.</>}</p>
      </section>
      ...
      <section className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-5">
        ...
        <p className="text-text-secondary text-xs leading-relaxed">{isHi ? <>JD और T सूर्य स्थिति सूत्रों में कैसे प्रयुक्त होते हैं इसके लिए <span className="text-gold-light">मॉड्यूल 22.2 (सूर्य भोगांश)</span> देखें। चन्द्र स्थिति के लिए <span className="text-gold-light">मॉड्यूल 22.3 (चन्द्र भोगांश)</span> देखें। सूर्योदय गणना (जो JD आंशिक-दिन सटीकता पर निर्भर करती है) के लिए <span className="text-gold-light">मॉड्यूल 22.4</span> देखें। JD-आधारित वार गणना के लिए <span className="text-gold-light">मॉड्यूल 7.3 (वार)</span> देखें।</> : <>For how JD and T feed into Sun position formulas, see <span className="text-gold-light">Module 22.2 (Sun Longitude)</span>. For the Moon&apos;s position, see <span className="text-gold-light">Module 22.3 (Moon Longitude)</span>. For sunrise computation (which relies on JD fractional-day precision), see <span className="text-gold-light">Module 22.4</span>. For JD-based weekday (vara) computation, see <span className="text-gold-light">Module 7.3 (Vara)</span>.</>}</p>
      </section>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      <section>
        ...
        <p className="text-text-secondary text-sm leading-relaxed mb-3">{isHi ? <>JavaScript 64-बिट IEEE 754 फ्लोटिंग-पॉइंट प्रयोग करता है, जो ~15-16 दशमलव अंकों की सटीकता देता है। JD 2461132.0 जैसी बड़ी संख्या में, दशमलव के बाद ~9 अंकों की सटीकता उपलब्ध है। यह ~0.000001 दिन = ~0.086 सेकण्ड की सटीकता है &mdash; खगोलीय गणना के लिए पर्याप्त से अधिक। JD गणना की सटीकता खगोलीय इंजन की सटीकता से अधिक है — स्विस एफेमेरिस से ग्रह स्थितियाँ उप-आर्क-सेकंड तक सटीक हैं (मीयस फ़ॉलबैक सूर्य ~0.01° / चन्द्र ~0.3°)।</> : <>JavaScript uses 64-bit IEEE 754 floating point, providing ~15-16 decimal digits of precision. For a number like JD 2461132.0, about ~9 digits of precision remain after the decimal point. This gives ~0.000001 day = ~0.086 seconds of precision &mdash; more than sufficient for astronomical computation. JD precision comfortably exceeds engine precision — Swiss Ephemeris (our primary) delivers sub-arcsecond planetary positions; the Meeus fallback is ~0.01° Sun, ~0.3° Moon.</>}</p>
      </section>
      <section className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-5 border border-red-500/15">
        <h4 className="text-red-400 text-xs uppercase tracking-widest font-bold mb-3">{tl({ en: 'Additional Misconceptions', hi: 'अतिरिक्त भ्रान्तियाँ', sa: 'अतिरिक्त भ्रान्तियाँ' }, locale)}</h4>
