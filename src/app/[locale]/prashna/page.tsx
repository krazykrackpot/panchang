...
// ── Trilingual labels ──
const L = {
  title: { en: 'Prashna Kundali', hi: 'प्रश्न कुण्डली', sa: 'प्रश्नकुण्डली', ta: 'பிரச்ன ஜாதகம்' },
  subtitle: {
    ...
    ta: 'வேத ஹோரா ஜோதிடம் — இந்த தருணத்தில் நட்சத்திரங்களிலிருந்து பதில்கள்',
  },
  whatIs: { en: 'What is Prashna?', hi: 'प्रश्न कुण्डली क्या है?', sa: 'प्रश्नकुण्डली किम्?', ta: 'பிரச்னம் என்றால் என்ன?' },
  whatIsDesc: {
  ...
  howTo: { en: 'How It Works', hi: 'यह कैसे काम करता है', sa: 'कथं कार्यं करोति', ta: 'இது எப்படி வேலை செய்கிறது' },
  ...
  question: { en: 'Your Question', hi: 'आपका प्रश्न', sa: 'भवतः प्रश्नः', ta: 'உங்கள் கேள்வி' },
  questionPlaceholder: {
  ...
  selectCategory: { en: 'Select Category', hi: 'श्रेणी चुनें', sa: 'वर्गं चिनुयात्', ta: 'வகையைத் தேர்ந்தெடுக்கவும்' },
  castBtn: { en: 'Cast Prashna Chart', hi: 'प्रश्न कुण्डली बनाएं', sa: 'प्रश्नकुण्डलीं रचयेत्', ta: 'பிரச்ன ஜாதகம் உருவாக்கவும்' },
  casting: { en: 'Casting Chart...', hi: 'कुण्डली बन रही है...', sa: 'कुण्डली रच्यते...', ta: 'ஜாதகம் உருவாகிறது...' },
  castAt: { en: 'Chart cast at', hi: 'कुण्डली बनाई गई', sa: 'कुण्डली रचिता' },
  verdict: { en: 'Verdict', hi: 'निर्णय', sa: 'निर्णयः', ta: 'தீர்ப்பு' },
  keyIndicators: { en: 'Key Indicators', hi: 'प्रमुख सूचक', sa: 'प्रमुखसूचकाः', ta: 'முக்கிய குறிகாட்டிகள்' },
  chart: { en: 'Prashna Chart', hi: 'प्रश्न कुण्डली', sa: 'प्रश्नकुण्डली', ta: 'பிரச்ன ஜாதகம்' },
  north: { en: 'North Indian', hi: 'उत्तर भारतीय', sa: 'उत्तरभारतीयम्' },
  south: { en: 'South Indian', hi: 'दक्षिण भारतीय', sa: 'दक्षिणभारतीयम्' },
  factors: { en: 'Detailed Analysis', hi: 'विस्तृत विश्लेषण', sa: 'विस्तृतविश्लेषणम्', ta: 'விரிவான பகுப்பாய்வு' },
  ...
  remedies: { en: 'Remedies', hi: 'उपाय', sa: 'उपायाः', ta: 'பரிகாரங்கள்' },
  ...
  newChart: { en: 'Ask Another Question', hi: 'दूसरा प्रश्न पूछें', sa: 'अन्यं प्रश्नं पृच्छेत्', ta: 'மற்றொரு கேள்வி கேளுங்கள்' },
...
export default function PrashnaPage() {
  ...
  const castPrashna = useCallback(async () => {
    ...
    setLoading(true);
    const now = new Date();
    setCastTime(now.toLocaleString(!isDevanagariLocale(locale) ? 'en-IN' : 'hi-IN'));
    ...
    try {
      const res = await authedFetch('/api/kundali', {
        method: 'POST',
        body: JSON.stringify({
          name: !isDevanagariLocale(locale) ? 'Prashna Chart' : 'प्रश्न कुण्डली',
              ...
              <div className="flex justify-center">
                <div className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-4 inline-block">
                  <div className="flex items-center justify-center gap-2 mb-2 text-xs text-text-secondary">
                    ...
                    <span style={bf}>
                      {!isDevanagariLocale(locale) ? 'Ascendant' : 'लग्न'}: {kundali.ascendant.signName[locale]} ({kundali.ascendant.degree.toFixed(1)}&deg;)
                    </span>
                  </div>
                  {chartStyle === 'north' ? (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gold-primary/10 bg-bg-tertiary/30">
                        <th className="text-left px-4 py-3 text-text-secondary font-bold text-xs uppercase">{!isDevanagariLocale(locale) ? 'Planet' : 'ग्रह'}</th>
                        <th className="text-left px-4 py-3 text-text-secondary font-bold text-xs uppercase">{!isDevanagariLocale(locale) ? 'Sign' : 'राशि'}</th>
                        <th className="text-center px-4 py-3 text-text-secondary font-bold text-xs uppercase">{!isDevanagariLocale(locale) ? 'House' : 'भाव'}</th>
                        <th className="text-left px-4 py-3 text-text-secondary font-bold text-xs uppercase">{!isDevanagariLocale(locale) ? 'Status' : 'स्थिति'}</th>
                        <th className="text-left px-4 py-3 text-text-secondary font-bold text-xs uppercase">{!isDevanagariLocale(locale) ? 'Role' : 'भूमिका'}</th>
                      </tr>
                    </thead>
                    <tbody>
    ...
    <div className={`rounded-2xl border bg-gradient-to-b p-6 sm:p-8 ${cfg.color}`}>
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="ml-auto text-right">
          ...
          <div className="text-xs text-text-secondary uppercase">{!isDevanagariLocale(locale) ? 'Score' : 'अंक'}</div>
        </div>
      </div>
