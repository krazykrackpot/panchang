  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center">
        ...
        <p className="text-text-secondary">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Loading shared chart...' : 'साझा कुण्डली लोड हो रही है...'}</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-12">
        <Link href="/kundali" className="inline-flex items-center gap-2 text-gold-primary hover:text-gold-light mb-8">
          <ArrowLeft className="w-4 h-4" /> {(locale !== 'hi' && String(locale) !== 'sa') ? 'Generate New Chart' : 'नई कुण्डली बनाएं'}
        </Link>
        <div className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-12 text-center">
          <h1 className="text-3xl text-red-400 font-bold mb-4" style={headingFont}>
            {(locale !== 'hi' && String(locale) !== 'sa') ? 'Invalid Chart Link' : 'अमान्य कुण्डली लिंक'}
          </h1>
          ...
          <Link href="/kundali" className="inline-block mt-6 px-6 py-3 bg-gradient-to-r from-gold-dark to-gold-primary text-bg-primary font-semibold rounded-lg">
            {(locale !== 'hi' && String(locale) !== 'sa') ? 'Create Your Own Chart' : 'अपनी कुण्डली बनाएं'}
          </Link>
        </div>
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleCopy} className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-lg text-sm text-gold-primary hover:text-gold-light">
            ...
            {copied ? ((locale !== 'hi' && String(locale) !== 'sa') ? 'Copied!' : 'कॉपी!') : ((locale !== 'hi' && String(locale) !== 'sa') ? 'Copy Link' : 'लिंक कॉपी')}
          </button>
          {typeof navigator !== 'undefined' && 'share' in navigator && (
            <button onClick={() => navigator.share({ title: `${chartData.name} - Kundali`, url: shareUrl })}
              ...
              <Share2 className="w-4 h-4" /> {(locale !== 'hi' && String(locale) !== 'sa') ? 'Share' : 'साझा'}
            </button>
          )}
        </div>
      </div>
        ...
        {kundali && (
          <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="p-3 bg-bg-primary/30 rounded-lg text-center">
              <p className="text-text-tertiary text-xs mb-1">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Ascendant' : 'लग्न'}</p>
              <p className="text-gold-light font-semibold">{kundali.ascendant.signName[locale]}</p>
            </div>
            <div className="p-3 bg-bg-primary/30 rounded-lg text-center">
              <p className="text-text-tertiary text-xs mb-1">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Moon Sign' : 'चंद्र राशि'}</p>
              <p className="text-gold-light font-semibold">{kundali.planets.find(p => p.planet.id === 1)?.signName[locale] || '-'}</p>
            </div>
            <div className="p-3 bg-bg-primary/30 rounded-lg text-center">
              <p className="text-text-tertiary text-xs mb-1">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Sun Sign' : 'सूर्य राशि'}</p>
              <p className="text-gold-light font-semibold">{kundali.planets.find(p => p.planet.id === 0)?.signName[locale] || '-'}</p>
            </div>
            <div className="p-3 bg-bg-primary/30 rounded-lg text-center">
              <p className="text-text-tertiary text-xs mb-1">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Nakshatra' : 'नक्षत्र'}</p>
      ...
      {kundali?.chart && (
        <div className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-8 mb-8 flex flex-col items-center">
          <h2 className="text-xl text-gold-gradient font-bold mb-6 self-start" style={headingFont}>
            {(locale !== 'hi' && String(locale) !== 'sa') ? 'Birth Chart (D1 — Rashi)' : locale === 'hi' ? 'जन्म कुण्डली (D1 — राशि)' : 'जन्मकुण्डली (D1 — राशिः)'}
          </h2>
      ...
      {kundali && (
        <div className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-8 mb-8">
          <h2 className="text-xl text-gold-gradient font-bold mb-6" style={headingFont}>
            {(locale !== 'hi' && String(locale) !== 'sa') ? 'Planetary Positions' : 'ग्रह स्थिति'}
          </h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-gold-primary border-b border-gold-primary/10">
                  <th className="text-left py-2 px-3">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Planet' : 'ग्रह'}</th>
                  <th className="text-left py-2 px-3">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Sign' : 'राशि'}</th>
                  <th className="text-right py-2 px-3">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Degree' : 'अंश'}</th>
                  <th className="text-center py-2 px-3">{(locale !== 'hi' && String(locale) !== 'sa') ? 'House' : 'भाव'}</th>
                  <th className="text-center py-2 px-3">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Nakshatra' : 'नक्षत्र'}</th>
                </tr>
              </thead>
              <tbody>
        ...
        return (
          <div className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-8 mb-8">
            <h2 className="text-xl text-gold-gradient font-bold mb-2" style={headingFont}>
              {(locale !== 'hi' && String(locale) !== 'sa') ? 'Recommended Graha Shanti Pujas' : 'अनुशंसित ग्रह शान्ति पूजा'}
            </h2>
            ...
            <div className="space-y-3">
              {afflicted.map((ap) => {
                const cfg = {
                  severe: { border: 'border-rose-500/20', bg: 'bg-rose-500/8', text: 'text-rose-400', badge: 'bg-rose-500/20 text-rose-400', label: (locale !== 'hi' && String(locale) !== 'sa') ? 'Severe' : 'गम्भीर' },
                  moderate: { border: 'border-amber-500/20', bg: 'bg-amber-500/8', text: 'text-amber-400', badge: 'bg-amber-500/20 text-amber-400', label: (locale !== 'hi' && String(locale) !== 'sa') ? 'Moderate' : 'मध्यम' },
                  mild: { border: 'border-blue-500/20', bg: 'bg-blue-500/8', text: 'text-blue-400', badge: 'bg-blue-500/20 text-blue-400', label: (locale !== 'hi' && String(locale) !== 'sa') ? 'Mild' : 'साधारण' },
                }[ap.severity];
                ...
                return (
                  <div key={ap.planetId} className={`p-4 rounded-xl border ${cfg.border} ${cfg.bg}`}>
                    ...
                    >
                      {(locale !== 'hi' && String(locale) !== 'sa') ? 'View Puja Details' : 'पूजा विवरण देखें'}
      ...
      <div className="text-center mt-8">
        <p className="text-text-secondary mb-4">
          {(locale !== 'hi' && String(locale) !== 'sa') ? 'Want a full analysis with Dashas, Yogas, and more?' : 'पूर्ण दशा, योग और अधिक विश्लेषण चाहते हैं?'}
        </p>
        <Link href="/kundali" className="inline-block px-8 py-3 bg-gradient-to-r from-gold-dark to-gold-primary text-bg-primary font-semibold rounded-lg hover:from-gold-primary hover:to-gold-light transition-all">
          {(locale !== 'hi' && String(locale) !== 'sa') ? 'Generate Full Chart' : 'पूर्ण कुण्डली बनाएं'}
        </Link>
      </div>
    </div>
  );
