    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          '**Exact fallback trigger conditions (from the code):** The engine drops to Meeus in three specific cases: (1) running in a browser context (`typeof window !== "undefined"`) — the `sweph` native binding is server-only; (2) `require("sweph")` throws — the native binary is missing or incompatible on the deployment target; (3) an individual Swiss Ephemeris call returns `null` for a genuine mathematical reason — most commonly the Placidus `houses_ex2` routine at extreme polar latitudes (|lat| ≥ ~89.5°). In production on Vercel, path (1) is inapplicable (server-side render) and paths (2)–(3) are exceedingly rare. If you ever see a Meeus fallback warning on a chart, it means one of the three conditions above fired for that specific request.',
      ...
      },
      {
        ...
        body: [
          '**Sunrise/Sunset Algorithm:** We use Swiss Ephemeris\'s `rise_trans` routine for sunrise and sunset — solving directly for the moment the upper limb of the solar disc crosses the geometrical horizon after applying atmospheric refraction of exactly 34\' (arcminutes). This matches the classical definition used in the Indian panchang tradition. When Swiss Ephemeris is unavailable, the engine falls back to Meeus\'s 2-pass algorithm with the full geometric solar position (not simplified tables) plus the same 34\' refraction constant.',
      ...
      {
        id: 'inauspicious-periods',
        icon: 'shield',
        heading: 'Rahu Kaal, Yamaganda & Gulika Kaal',
        body: [
          '**Vara-based construction.** All three inauspicious periods are computed from the same underlying interval: **one-eighth of the daytime span (sunrise → sunset)**. The daytime is divided into 8 equal muhurta slots numbered 0–7. Which slot each period occupies is fixed by the day of the week (Vara), matching the tables in classical Muhurta texts.',
          '**Rahu Kaal slot per weekday** (0 = first slot after sunrise, 7 = last before sunset): Sunday → 7 (but by convention the traditional Rahu Kaal is reckoned starting from the count position, not the index — see the reference table in `src/lib/muhurta/inauspicious-periods.ts` for the exact mapping). Monday → 1, Tuesday → 6, Wednesday → 4, Thursday → 5, Friday → 3, Saturday → 2.',
          '**Yamaganda Kaal** occupies its own weekday-indexed slot from the same 8-slot division, on a different sequence. **Gulika Kaal** (also called Maandi) uses a third sequence. All three are canonical Vara-based derivations — the same formula every classical panchang publishes — and all three are exposed by `calculateRahuKaal()`, `calculateYamaganda()`, and `calculateGulika()` in `src/lib/ephem/astronomical.ts`.',
          '**Precision.** Because the segment length depends only on the day-length, precision comes down to sunrise and sunset. Since sunrise/sunset are computed by Swiss Ephemeris (Meeus fallback) with 34\' refraction and topocentric correction, Rahu Kaal boundaries are accurate to the same tolerance as the sunrise itself — comfortably within one minute at any populated latitude.',
          '**Midnight crossing.** For any activity that continues past sunset (e.g. Yamaganda\'s counterpart at night — Ratri Rahu Kaal), the wrap-aware `if (end < start) ...` comparison is used consistently. This prevents the "NOW badge never appears for a night slot" bug.',
        ],
      },
      {
        id: 'choghadiya',
        ...
        heading: 'Choghadiya',
        body: [
          '**Definition.** Choghadiya divides the day and night each into 8 sub-slots of equal length — a total of 16 slots per 24-hour Vedic day. Each slot inherits a quality (Amrit, Shubh, Labh, Chal for auspicious; Rog, Kaal, Udveg, Vaar for inauspicious) determined by (a) whether it\'s a daytime or nighttime slot, (b) its position in the sequence, and (c) the weekday.',
          '**Segment length.** Day slots: (sunset − sunrise) ÷ 8. Night slots: (next sunrise − sunset) ÷ 8. Because summer and winter days differ by 4+ hours at mid-latitudes, Choghadiya slot durations vary between ~30 min (deep winter) and ~2 h (near solstice at northern latitudes). We compute the actual sunrise/sunset for the specific date and coordinates — no fixed 90-minute approximation.',
          '**Weekday sequence.** The starting quality of the first day slot is fixed by the Vara (weekday) per the classical Muhurta Chintamani table: Sunday starts with Udveg, Monday with Amrit, Tuesday with Rog, and so on. The remaining 7 daytime qualities follow a fixed cyclic order. Night sequence starts one step ahead of the day sequence. Both cycles are implemented in `src/lib/hora/hora-calculator.ts`.',
          '**Overrides.** When a Choghadiya slot overlaps a Rahu Kaal / Yamaganda / Gulika / Varjyam interval, the UI overlays an amber warning marker. The auspicious label is not silently replaced — the user sees both, and Muhurta guidance recommends avoiding the conflict window regardless of the base slot\'s quality.',
        ],
      },
      {
        id: 'timezone-dst',
        icon: 'clock',
        heading: 'Timezone & DST Handling (Runtime)',
        body: [
          '**Never trust the browser.** Panchang windows are location-specific: sunrise in Chennai and sunrise in Delhi differ by more than 30 minutes on the same UTC instant. The engine ignores the browser\'s `Date.getTimezoneOffset()` entirely for panchang and kundali computations. The IANA tzdb offset is resolved from the birth or panchang coordinates instead, per-date.',
          '**Per-date resolution.** DST transitions can shift a coordinate\'s offset by ±1 hour. A panchang computed on 2026-10-25 (the day EU clocks fall back) uses two different offsets before and after the transition instant. The engine looks up the IANA offset for the specific UTC instant of each event (sunrise, tithi boundary, muhurta start), not a single "offset for the day".',
          '**Post-1880 births use IANA tzdb.** For any date within the range the tzdb defines (~1880 onward), timezone resolution uses `@vvo/tzdb` (or equivalent), keyed by IANA zone id. This correctly handles historical zone reforms — e.g. India Standard Time was introduced in 1906, before which each Indian city kept its own local mean time.',
          '**Pre-1880 births use longitude-derived Local Mean Time.** See the "Historical Timezone Handling" section below for full details.',
          '**Explicit UTC everywhere in code.** Every `Date` constructor in the computation modules uses `Date.UTC(y, m-1, d, h, m)` or `.getTime() + ms`. Naked `new Date(year, month-1, day)` is banned because it interprets arguments in the *server\'s* timezone, which produces different values on Vercel (UTC) vs. on a developer laptop in CET.',
        ],
      },
      {
