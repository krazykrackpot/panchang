    ...
    <div>
      ...
      <LessonSection title={L.abhijitTitle[locale]}>
        ...
        <div className="mt-4 p-4 bg-bg-primary/50 rounded-lg border border-gold-primary/30">
          <p className="text-gold-light font-mono text-sm mb-1">
            {locale === 'en' ? 'Abhijit Window Calculation:' : 'अभिजित् गणना:'}
          </p>
          ...
          <p className="text-gold-light/60 font-mono text-xs mt-2">
            {locale === 'en' ? 'Typically ~11:36 AM to 12:24 PM at equinox (varies by latitude/season)' : 'विषुव पर सामान्यतः ~11:36 से 12:24 (अक्षांश/ऋतु से भिन्न)'}
          </p>
        </div>
      </LessonSection>
      ...
      <LessonSection title={L.brahmaTitle[locale]}>
        ...
        <div className="mt-4 p-4 bg-bg-primary/50 rounded-lg border border-indigo-400/20">
          <p className="text-indigo-300 font-mono text-sm mb-1">
            {locale === 'en' ? 'Brahma Muhurta Timing:' : 'ब्रह्म मुहूर्त समय:'}
          </p>
        ...
        <div className="mt-4 overflow-x-auto bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-4 border border-red-500/15">
          ...
          <table className="w-full text-xs">
            <thead><tr className="border-b border-red-500/15">
              <th className="text-left py-2 px-2 text-gold-dark">{locale === 'en' ? 'Day' : 'दिन'}</th>
              <th className="text-left py-2 px-2 text-red-300">{locale === 'en' ? 'Rahu Kaal' : 'राहु काल'}</th>
              <th className="text-left py-2 px-2 text-amber-300">{locale === 'en' ? 'Yamaganda' : 'यमगण्ड'}</th>
              <th className="text-left py-2 px-2 text-purple-300">{locale === 'en' ? 'Gulika' : 'गुलिक'}</th>
            </tr></thead>
            <tbody className="divide-y divide-gold-primary/5 text-text-secondary">
              <tr><td className="py-1.5 px-2">{locale === 'en' ? 'Sunday' : 'रवि'}</td><td className="py-1.5 px-2">8th</td><td className="py-1.5 px-2">5th</td><td className="py-1.5 px-2">7th</td></tr>
              <tr><td className="py-1.5 px-2">{locale === 'en' ? 'Monday' : 'सोम'}</td><td className="py-1.5 px-2">2nd</td><td className="py-1.5 px-2">4th</td><td className="py-1.5 px-2">6th</td></tr>
              <tr><td className="py-1.5 px-2">{locale === 'en' ? 'Tuesday' : 'मंगल'}</td><td className="py-1.5 px-2">7th</td><td className="py-1.5 px-2">3rd</td><td className="py-1.5 px-2">5th</td></tr>
              <tr><td className="py-1.5 px-2">{locale === 'en' ? 'Wednesday' : 'बुध'}</td><td className="py-1.5 px-2">5th</td><td className="py-1.5 px-2">2nd</td><td className="py-1.5 px-2">4th</td></tr>
              <tr><td className="py-1.5 px-2">{locale === 'en' ? 'Thursday' : 'गुरु'}</td><td className="py-1.5 px-2">6th</td><td className="py-1.5 px-2">1st</td><td className="py-1.5 px-2">3rd</td></tr>
              <tr><td className="py-1.5 px-2">{locale === 'en' ? 'Friday' : 'शुक्र'}</td><td className="py-1.5 px-2">4th</td><td className="py-1.5 px-2">7th</td><td className="py-1.5 px-2">2nd</td></tr>
              <tr><td className="py-1.5 px-2">{locale === 'en' ? 'Saturday' : 'शनि'}</td><td className="py-1.5 px-2">3rd</td><td className="py-1.5 px-2">6th</td><td className="py-1.5 px-2">1st</td></tr>
            </tbody>
          </table>
        </div>
      </LessonSection>
      ...
      <LessonSection title={L.choghadiyaTitle[locale]}>
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            { name: 'Amrit', planet: locale === 'en' ? 'Jupiter' : 'गुरु', nature: 'best', color: 'text-emerald-400 border-emerald-500/20 bg-emerald-500/5' },
            { name: 'Shubh', planet: locale === 'en' ? 'Venus' : 'शुक्र', nature: 'good', color: 'text-emerald-300 border-emerald-500/15 bg-emerald-500/3' },
            { name: 'Labh', planet: locale === 'en' ? 'Mercury' : 'बुध', nature: 'good', color: 'text-green-300 border-green-500/15 bg-green-500/3' },
            { name: 'Char', planet: locale === 'en' ? 'Moon' : 'चन्द्र', nature: 'travel', color: 'text-amber-300 border-amber-500/15 bg-amber-500/3' },
            { name: 'Udveg', planet: locale === 'en' ? 'Sun' : 'सूर्य', nature: 'avoid', color: 'text-red-300 border-red-500/15 bg-red-500/3' },
            { name: 'Kaal', planet: locale === 'en' ? 'Saturn' : 'शनि', nature: 'avoid', color: 'text-red-400 border-red-500/20 bg-red-500/5' },
            { name: 'Rog', planet: locale === 'en' ? 'Mars' : 'मंगल', nature: 'avoid', color: 'text-red-300 border-red-500/15 bg-red-500/3' },
          ].map((ch) => (
      ...
      <LessonSection title={L.horaTitle[locale]}>
        ...
        <div className="mt-4 p-4 bg-bg-primary/50 rounded-lg border border-gold-primary/10">
          <p className="text-gold-light font-mono text-sm mb-2">
            {locale === 'en' ? 'Chaldean Order (descending orbital period):' : 'कल्डियन क्रम (अवरोही कक्षा अवधि):'}
          </p>
          ...
          <p className="text-gold-light/60 font-mono text-xs mt-3">
            {locale === 'en' ? 'Why does Sunday follow Saturday?' : 'शनिवार के बाद रविवार क्यों?'}
          </p>
      ...
      <LessonSection title={L.selectingTitle[locale]}>
        ...
        <div className="mt-4 p-4 bg-bg-primary/50 rounded-lg border border-emerald-400/20">
          <p className="text-emerald-300 font-mono text-sm mb-2">
            {locale === 'en' ? 'Quick Muhurta Checklist:' : 'त्वरित मुहूर्त जाँचसूची:'}
          </p>
      ...
      <div className="mt-8 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {[
            { label: locale === 'en' ? 'Muhurta AI — Find Auspicious Times' : 'मुहूर्त AI — शुभ समय खोजें', href: '/muhurta-ai', color: 'border-emerald-500/20 hover:border-emerald-500/40' },
            { label: locale === 'en' ? 'Daily Panchang' : 'दैनिक पंचांग', href: '/panchang', color: 'border-gold-primary/20 hover:border-gold-primary/40' },
            { label: locale === 'en' ? 'Muhurta Wheel & Conflict Analysis' : 'मुहूर्त चक्र और विरोध विश्लेषण', href: '/panchang/muhurta', color: 'border-gold-primary/20 hover:border-gold-primary/40' },
            { label: locale === 'en' ? 'Module 17-1: Muhurta Foundations' : 'मॉड्यूल 17-1: मुहूर्त आधार', href: '/learn/modules/17-1', color: 'border-blue-500/20 hover:border-blue-500/40' },
            { label: locale === 'en' ? 'Module 17-2: Activity-Specific Rules' : 'मॉड्यूल 17-2: कार्य-विशिष्ट नियम', href: '/learn/modules/17-2', color: 'border-blue-500/20 hover:border-blue-500/40' },
            { label: locale === 'en' ? 'Module 17-3: Chandra & Tara Balam' : 'मॉड्यूल 17-3: चन्द्र और तारा बलम', href: '/learn/modules/17-3', color: 'border-blue-500/20 hover:border-blue-500/40' },
            { label: locale === 'en' ? 'Module 17-4: Panchaka & Lagna Shuddhi' : 'मॉड्यूल 17-4: पंचक और लग्न शुद्धि', href: '/learn/modules/17-4', color: 'border-blue-500/20 hover:border-blue-500/40' },
          ].map((link) => (
            <Link
              key={link.href}
              href={link.href}
