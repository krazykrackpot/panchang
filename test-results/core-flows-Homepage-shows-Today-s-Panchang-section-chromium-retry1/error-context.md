...
Expected: visible
Error: strict mode violation: locator('text=Panchang') resolved to 10 elements:
    ...
    6) <span class="text-gold-light text-sm">Set your location to see Panchang</span> aka getByRole('button', { name: 'Set your location to see' })
    7) <p class="text-text-secondary text-lg mb-2">Location required for accurate Panchang</p> aka getByText('Location required for')
    8) <p class="text-text-secondary/60 text-sm mb-6">Panchang calculations depend on sunrise/sunset at…</p> aka getByText('Panchang calculations depend')
    9) <span class="text-gold-primary/60 text-sm font-semibold">Dekho Panchang</span> aka getByRole('contentinfo').getByText('Dekho Panchang')
    10) <a href="/en/panchang" class="hover:text-gold-light transition-colors">Panchang</a> aka getByRole('contentinfo').getByRole('link', { name: 'Panchang' })
