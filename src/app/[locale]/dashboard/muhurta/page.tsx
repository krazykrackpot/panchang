  ...
  if (!user) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        <p className="text-text-secondary">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Sign in to see personal muhurta' : 'व्यक्तिगत मुहूर्त देखने के लिए साइन इन करें'}</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-4xl font-bold mb-3" style={hf}><span className="text-gold-gradient">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Personal Muhurta' : 'व्यक्तिगत मुहूर्त'}</span></h1>
            ...
            <motion.div key={m.activityKey} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="flex items-center gap-4 mb-2 text-xs">
                <span className="flex items-center gap-1">
                  {(locale !== 'hi' && String(locale) !== 'sa') ? 'Tara' : 'तारा'}: {m.taraBala.favorable ? <Check className="w-3 h-3 text-emerald-400" /> : <X className="w-3 h-3 text-red-400" />}
                  <span className="text-text-secondary">{m.taraBala.taraName}</span>
                </span>
                <span className="flex items-center gap-1">
                  {(locale !== 'hi' && String(locale) !== 'sa') ? 'Chandra' : 'चन्द्र'}: {m.chandraBala.favorable ? <Check className="w-3 h-3 text-emerald-400" /> : <X className="w-3 h-3 text-red-400" />}
      ...
      <div className="mt-8 bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-5 text-xs text-text-secondary" style={bf}>
        <p className="text-gold-dark font-bold uppercase tracking-wider mb-2">{(locale !== 'hi' && String(locale) !== 'sa') ? 'How this works' : 'यह कैसे काम करता है'}</p>
        <p>{(locale !== 'hi' && String(locale) !== 'sa')
