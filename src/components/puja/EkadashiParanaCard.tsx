...
import MSG from '@/messages/components/ekadashi-parana.json';
...
function formatDateLocale(dateStr: string, locale: Locale): string {
  ...
  const loc = msg('dateLocaleFmt', locale);
  try {
        ...
        <div className="mb-5 grid grid-cols-3 gap-3 text-center">
          ...
          {dwadashiEndTime && (
            <div className="rounded-lg border border-rose-500/15 bg-rose-500/[0.04] p-3">
              ...
              <div className="text-xs text-rose-400/50 mt-1">{msg('absoluteDeadline', locale)}</div>
            </div>
          )}
        </div>
      )}
