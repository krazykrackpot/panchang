// src/lib/kundali/health-diagnosis/__tests__/elements/digestive.test.ts
//
// Task B3 — digestive element scorer tests.
//
...
import { scoreDigestive } from '../../elements/digestive';
...
const result = scoreDigestive(kundali, strength, signatures, 'en');

describe('scoreDigestive', () => {
  ...
  it('returns correct metadata matching ELEMENT_CATALOG[digestive]', () => {
    const meta = ELEMENT_CATALOG['digestive'];
    expect(result.id).toBe('digestive');
