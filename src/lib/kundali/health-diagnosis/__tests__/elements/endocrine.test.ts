// src/lib/kundali/health-diagnosis/__tests__/elements/endocrine.test.ts
//
// Task B12 — endocrine element scorer tests.
// endocrine has badge === 'inferential' — test asserts this.
//
...
import { scoreEndocrine } from '../../elements/endocrine';
...
const result = scoreEndocrine(kundali, strength, signatures, 'en');

describe('scoreEndocrine', () => {
  it('returns a NatalElement with score in [0,100], valid rating, and at least 4 factors', () => {
    ...
    // endocrine is an inferential element
    expect(result.badge).toBe('inferential');
  });

  it('returns correct metadata matching ELEMENT_CATALOG[endocrine]', () => {
    const meta = ELEMENT_CATALOG['endocrine'];
    expect(result.id).toBe('endocrine');
