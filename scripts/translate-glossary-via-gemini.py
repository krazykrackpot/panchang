#!/usr/bin/env python3
"""
Translate GLOSSARY (45 Jyotish term definitions) to 7 visible
regional locales via Gemini 2.5 Flash on Vertex AI.

Output: src/lib/constants/glossary-{locale}-overlay.json, keyed by
"<id>.<field>".
...
JOBS_FILE = Path("/tmp/glossary-jobs.json")
...
def _write_overlay(locale: str, overlay: dict[str, str]) -> None:
    out_path = OUT_DIR / f"glossary-{locale}-overlay.json"
    merged: dict[str, str] = {}
    if out_path.exists():
        try:
