#!/usr/bin/env python3
"""
Translate DREKKANA_FACES (36 D3 faces × image + interpretation) to 7 visible
regional locales via Gemini 2.5 Flash on Vertex AI.

Output: src/lib/constants/drekkana-faces-{locale}-overlay.json, keyed by
"<signId>-<decanate>.<field>".
...
JOBS_FILE = Path("/tmp/drekkana-jobs.json")
...
def _write_overlay(locale: str, overlay: dict[str, str]) -> None:
    out_path = OUT_DIR / f"drekkana-faces-{locale}-overlay.json"
    merged: dict[str, str] = {}
    if out_path.exists():
        try:
