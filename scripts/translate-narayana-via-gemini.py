#!/usr/bin/env python3
"""
Translate NARAYANA_INTERPRETATIONS (12 signs × themes/focus/caution Jaimini dasha) to 7 visible
regional locales via Gemini 2.5 Flash on Vertex AI.

Output: src/lib/constants/narayana-interpretations-{locale}-overlay.json, keyed by
"<rashiId>.<field>".
...
JOBS_FILE = Path("/tmp/narayana-jobs.json")
...
def _write_overlay(locale: str, overlay: dict[str, str]) -> None:
    out_path = OUT_DIR / f"narayana-interpretations-{locale}-overlay.json"
    merged: dict[str, str] = {}
    if out_path.exists():
        try:
