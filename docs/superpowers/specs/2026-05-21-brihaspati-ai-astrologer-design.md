...
create table public.brihaspati_questions (
  ...
  input_tokens int,
  output_tokens int,
  -- training-data flywheel (§11):
  context_json jsonb,                        -- structured Layer-2 context that was sent to the LLM
  engine_version text,                       -- ENGINE_VERSION at the time of computation
  system_prompt_version text,                -- sha256(systemPrompt) so we can group by prompt revision
  user_rating smallint,                      -- -1 thumbs-down, 0 unrated, 1 thumbs-up
  user_rating_reason text,                   -- free-text "what was wrong" if rating = -1
  training_eligible boolean,                 -- nightly job sets this per §11 filter rules
  training_opt_out boolean default false,    -- snapshot of user_profiles.brihaspati_training_opt_out at write time
...
create index brihaspati_questions_training on public.brihaspati_questions (training_eligible, query_category, locale)
...
-- user_profiles.brihaspati_training_opt_out boolean default false
-- ↑ added via the migration too; surfaced as a toggle on /settings (§11)
...
Every production question answered by Tier 2 (Claude API) becomes a labelled training pair. The Hetzner self-host decision is deferred — but the data pipeline is **not** deferred, so that when Phase 3 of INFRASTRUCTURE.md lands (Qwen fine-tuning), we already have months of real production pairs to train on. This turns the Claude-API launch from a stop-gap into the bootstrap for the eventual self-hosted model.

### What we capture per question

Every row in `brihaspati_questions` already stores `question` and `answer`. The flywheel adds five columns to that row (see schema in §Backend):

- `context_json` — the structured Layer-2 JSON sent to the LLM. **This is the critical column** — without it the answer is unreproducible and useless for training.
- `engine_version` — the `ENGINE_VERSION` hash at compute time. Lets us discard pairs whose context was generated by an engine that has since materially changed.
- `system_prompt_version` — `sha256` of the system prompt. Lets us regenerate-or-retrain when the prompt evolves.
- `user_rating` — `-1 | 0 | 1` from a thumbs-up/down control under the answer.
- `user_rating_reason` — free text, optional, shown only on thumbs-down.

### Training-eligibility filter

A nightly Vercel Cron job (`/api/cron/brihaspati-mark-eligible`) sets `training_eligible = true` for rows where ALL of:

- `validation_passed = true` (Layer 4 cleared)
- `status = 'completed'`
- `training_opt_out = false` (user did not opt out)
- `tier = 2` (Claude API — we don't want to bootstrap Qwen by training on its own template fallback or worse on its own outputs once it lands)
- `output_tokens between 200 and 800` (typical answer length; filters runts and run-aways)
- `user_rating >= 0` (any thumbs-down is dropped)
- `engine_version` is within the last 90 days (older pairs are quarantined for review)
- `created_at` is at least 7 days old (gives users time to ask for a refund / flag wrong answers before the pair enters training)

### Export pipeline

When we eventually run training (INFRASTRUCTURE.md Phase 3, on-demand):

1. **Export**: `scripts/brihaspati-train/export.ts` reads `training_eligible = true` rows, joins to user_profiles for locale, and emits `dataset.jsonl` in chat-format (`{messages: [{role: 'system', content: <system_prompt_at_version>}, {role: 'user', content: <question> + <context_json>}, {role: 'assistant', content: <answer>}]}`).
2. **Stratify**: balance across `query_category × locale` — over-represented categories get downsampled, under-represented get topped up with synthetic Claude-generated pairs.
3. **Train / eval split**: 90 / 10 by `user_id` (never split within a user — same person's questions cluster).
4. **Hold-out canary**: 50 specific (chart, question) pairs kept out of training, used to detect regressions across re-training runs.

### Privacy + opt-out

This is user data, even if anonymised. Three guardrails:

- **Opt-out toggle** in `/settings`: "Allow my anonymised questions and chart context to improve Brihaspati" (default on, can be turned off without affecting service). Drives `user_profiles.brihaspati_training_opt_out`. The flag is snapshotted onto the question row at write time so a future opt-out doesn't retroactively remove already-flagged-eligible rows — but opting out **does** stop new rows from becoming eligible.
- **Privacy policy** addendum: explicit clause stating production Q&A may be used to train internal models. The clause names the data classes: question text, chart context (planet positions, dashas, derived analysis), answer, validation result. PII (name, exact birth time/location) is **excluded** from training exports — only the derived chart context goes in. Implemented in the export script.
- **Right to erasure**: existing GDPR delete endpoint already cascades from `user_profiles` to `brihaspati_questions`. Deleted rows are also purged from any exported training datasets within 30 days (tracked via `deletion_ledger` table).

### What we DON'T capture

- No raw birth coordinates or timestamps in the training export — only the derived chart positions
- No payment information
- No user identifiers — the export drops `user_id`
- No conversation history across questions — each question stands alone

### Trigger for first training run

When `count(* where training_eligible = true) >= 3,000` and per-category coverage minimum is met (≥150 rows in each of the 12 categories × 4 locales). Until then we either keep using Claude API or, if we want to ship Tier 1 sooner, bootstrap with synthetic Claude-generated pairs as originally planned in PLAN.md §6.
...
- `brihaspati_answer_rated` (rating: -1 | 1, category, model, has_reason: bool) — feeds §11 flywheel
- `brihaspati_upsell_shown` / `_taken` (from_tier, to_tier)
- `brihaspati_answer_shared` (channel: whatsapp | email)
- `brihaspati_training_opt_out_toggled` (to: true | false)
...
- **Training-data flywheel (§11)**: `context_json` + ratings captured on every answer; thumbs-up/down control under answer; `/settings` opt-out toggle; nightly eligibility cron; privacy policy clause
...
- [ ] `/privacy` page updated with training-data clause (§11)
- [ ] `/settings` exposes `brihaspati_training_opt_out` toggle
- [ ] `/api/cron/brihaspati-mark-eligible` registered in `vercel.ts` crons (daily 02:00 UTC)
