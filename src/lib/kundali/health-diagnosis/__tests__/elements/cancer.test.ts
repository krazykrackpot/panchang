// src/lib/kundali/health-diagnosis/__tests__/elements/cancer.test.ts
// B21 — mixed badge, opt-in, requiresDisclaimer: true
import { describe, it, expect } from 'vitest';
import { generateKundali } from '@/lib/ephem/kundali-calc';
import type { BirthData } from '@/types/kundali';
import { collectStrengthInputs } from '../../strength-inputs';
import { detectAllSignatures } from '../../signatures';
import { scoreCancer } from '../../elements/cancer';
...
const result = scoreCancer(kundali, strength, signatures, 'en');

describe('scoreCancer', () => {
  it('returns a NatalElement with score in [0,100], valid rating, and at least 4 factors', () => {
    ...
    // cancer has mixed badge and requires disclaimer
    expect(result.badge).toBe('mixed');
    expect(result.requiresDisclaimer).toBe(true);
  });
  it('returns correct metadata matching ELEMENT_CATALOG[cancer]', () => {
    const meta = ELEMENT_CATALOG['cancer'];
    expect(result.id).toBe('cancer');
