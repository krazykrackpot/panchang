...
- **Structural gate:** `e2e/isr-hydration-crawl.spec.ts` enumerates every ISR-cached `page.tsx` under `src/app/[locale]/`, navigates a sample URL per route in Playwright, and fails on any `pageerror` or console message matching `/#418|Hydration failed|Text content did not match/`. **Any new ISR page that adds a render-time clock call will fail this test in CI before merge.** Do not delete or skip this test to make a PR pass — the test catching it IS the prevention working.
- **Proactive check:** When adding `export const revalidate = N` to a page, grep the imported client components for `new Date(`, `Date.now(`, `todayInTimezone(` in render bodies. When adding a render-time clock call to a `'use client'` component, grep for which server pages import it and verify none are ISR-cached.
