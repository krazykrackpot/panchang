...
import MSG from '@/messages/pages/eclipses.json';
...
export default function EclipsesPage() {
  ...
  const formatDate = (dateStr: string) => {
    ...
    return d.toLocaleDateString(msg('localeDateLocale', locale), { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
  };
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
        ...
        <div className="mt-4 flex flex-col items-center gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-text-secondary text-sm">{effectiveName || msg('detectingLocation', locale)}</span>
            {overrideLoc && (
              <button
                ...
                title={msg('resetLocationTitle', locale)}
              >
        ...
        <a href={`/${locale}/learn/eclipses`} className="text-gold-primary/60 text-xs hover:text-gold-light transition-colors inline-flex items-center gap-1 mt-2">
          {msg('learnLink', locale)}
        </a>
        <div className="flex justify-center mt-4">
          <ShareRow
            pageTitle={msg('pageTitle', locale)}
            shareText={msg('shareText', locale)}
            locale={locale}
          />
        </div>
      </motion.div>
                      ...
                      {local && (
                        isVisible ? (
                          <span className="text-[10px] px-2.5 py-0.5 rounded-full font-bold bg-emerald-500/15 text-emerald-300 border border-emerald-500/25 flex items-center gap-1">
                            <Eye className="w-3 h-3" /> {msg('visible', locale)}
                          </span>
                        ) : (
                          <span className="text-[10px] px-2.5 py-0.5 rounded-full font-bold bg-red-500/10 text-red-400 border border-red-500/20 flex items-center gap-1">
                            <EyeOff className="w-3 h-3" /> {msg('notVisible', locale)}
                          </span>
                        )
                      )}
                    </div>
                      ...
                      <div className="px-6 sm:px-8 pb-6 sm:pb-8 space-y-5 border-t border-gold-primary/10 pt-5">
                            ...
                            <div className="rounded-xl bg-gradient-to-br from-[#2d1b69]/30 via-[#1a1040]/35 to-[#0a0e27] border border-gold-primary/10 p-5">
                              <div className="flex items-center gap-2 mb-4">
                                ...
                                <h4 className="text-sm font-bold text-gold-light uppercase tracking-wider" style={headingFont}>
                                  {msg('eclipseTimingsLocal', locale)}
                                </h4>
                              </div>
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                {isSolar ? (
                                  <>
                                    ...
                                    {local.eclipseStart && (
                                      <TimeCell label={msg('partialBeginsC1', locale)} time={local.eclipseStart} accent={accentColor} subtitle={msg('moonFirstTouchesSun', locale)} />
                                    )}
                                    {local.maximum && (
                                      <TimeCell label={msg('maximumEclipse', locale)} time={local.maximum} accent="text-gold-light" highlight subtitle={msg('peakCoverage', locale)} />
                                    )}
                                    <TimeCell
                                      label={
                                        local.endsAtSunset
                                          ? msg('endsAtSunset', locale)
                                          : msg('partialEndsC4', locale)
                                      }
                                      time={local.eclipseEnd || '--:--'}
                                      accent={accentColor}
                                      subtitle={local.endsAtSunset ? msg('eclipseInProgressAtSunset', locale) : msg('moonFullySeparates', locale)}
                                    />
                                  </>
                                ) : (
                                  <>
                                    ...
                                    {local.eclipseStart && (
                                      <TimeCell label={msg('penumbralBeginsP1', locale)} time={local.eclipseStart} accent="text-text-secondary/60" subtitle={msg('subtleDimmingStarts', locale)} />
                                    )}
                                    {local.partialStart && (
                                      <TimeCell label={msg('umbralBeginsU1', locale)} time={local.partialStart} accent={accentColor} subtitle={msg('darkShadowEntersMoon', locale)} />
                                    )}
                                    {local.maximum && (
                                      <TimeCell label={msg('maximumEclipse', locale)} time={local.maximum} accent="text-gold-light" highlight subtitle={msg('peakShadowCoverage', locale)} />
                                    )}
                                    {local.partialEnd && (
                                      <TimeCell label={msg('umbralEndsU2', locale)} time={local.partialEnd} accent={accentColor} subtitle={msg('darkShadowStartsToLeave', locale)} />
                                    )}
                                    <TimeCell
                                      label={
                                        local.endsAtMoonset
                                          ? msg('endsAtMoonset', locale)
                                          : msg('penumbralEndsP4', locale)
                                      }
                                      time={local.eclipseEnd || '--:--'}
                                      accent="text-text-secondary/60"
                                      subtitle={local.endsAtMoonset ? msg('eclipseInProgressAtMoonset', locale) : msg('moonFullyBright', locale)}
                                    />
                                  </>
                                )}
                              </div>
                              ...
                              <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-gold-primary/8">
                                {local.durationFormatted && (
                                  <div>
                                    <div className="text-[10px] text-text-secondary/50 uppercase tracking-wider">{msg('totalDuration', locale)}</div>
                                    <div className="text-sm text-gold-light font-mono font-bold">{local.durationFormatted}</div>
                                  </div>
                                )}
                                <div>
                                  <div className="text-[10px] text-text-secondary/50 uppercase tracking-wider">{msg('maxMagnitude', locale)}</div>
                                ...
                                {local.magnitudeAtSunset !== null && local.magnitudeAtSunset !== undefined && (
                                  <div>
                                    <div className="text-[10px] text-text-secondary/50 uppercase tracking-wider">{msg('magnitudeAtSunset', locale)}</div>
                                    <div className="text-sm text-gold-light font-mono font-bold">{local.magnitudeAtSunset.toFixed(2)}</div>
                                  </div>
                                )}
                                {local.sunrise && (
                                  <div>
                                    <div className="text-[10px] text-text-secondary/50 uppercase tracking-wider">{msg('sunrise', locale)}</div>
                                    <div className="text-sm text-text-secondary font-mono">{local.sunrise}</div>
                                  </div>
                                )}
                                {local.sunset && (
                                  <div>
                                    <div className="text-[10px] text-text-secondary/50 uppercase tracking-wider">{msg('sunset', locale)}</div>
                            ...
                            {local.sutakApplicable && (
                              <div className="rounded-xl bg-gradient-to-br from-[#2d1b69]/30 via-[#1a1040]/35 to-[#0a0e27] border border-amber-500/15 p-5">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-5">
                                  {local.sutakStart && (
                                    <TimeCell label={msg('sutakBeginsRecommended', locale)} time={local.sutakStart} accent="text-amber-400" highlight />
                                  )}
                                  {local.sutakEnd && (
                                    <TimeCell label={msg('sutakEnds', locale)} time={local.sutakEnd} accent="text-amber-400" />
                                  )}
                                  {local.sutakVulnerableStart && (
                                    <TimeCell
                                      label={msg('kidsOldSick', locale)}
                                      time={local.sutakVulnerableStart}
                                      accent="text-amber-400/70"
                                      subtitle={msg('sutakBeginsLabel', locale)}
                                    />
                                  )}
                                </div>
                                ...
                                <div className="border-t border-amber-500/10 pt-4">
                                  <p className="text-[10px] text-text-secondary/40 uppercase tracking-wider font-bold mb-3">
                                    {msg('sutakPerClassicalTexts', locale)}
                                  </p>
                                  <div className="space-y-2">
                                    {local.sutakTraditions.muhurtaChintamani && (
                                      <div className="flex items-center justify-between gap-3 px-3 py-2 rounded-lg bg-amber-500/5">
                                        <div className="min-w-0">
                                          <span className="text-xs text-amber-300 font-semibold">{msg('muhurtaChintamani', locale)}</span>
                                    ...
                                    {local.sutakTraditions.dharmaSindhu && (
                                      <div className="flex items-center justify-between gap-3 px-3 py-2 rounded-lg bg-amber-500/5">
                                        <div className="min-w-0">
                                          <span className="text-xs text-amber-300 font-semibold">{msg('dharmasindhu', locale)}</span>
                                    ...
                                    {local.sutakTraditions.nirnyaSindhu && (
                                      <div className="flex items-center justify-between gap-3 px-3 py-2 rounded-lg bg-amber-500/5">
                                        <div className="min-w-0">
                                          <span className="text-xs text-amber-300 font-semibold">{msg('nirnayaSindhu', locale)}</span>
                                  ...
                                  <p className="text-[10px] text-text-secondary/30 mt-2 italic">
                                    {msg('recommendedTimeNote', locale)}
                                  </p>
                                </div>
                              </div>
                            )}
                        ...
                        {!local && (
                          <div className="text-center py-4 text-text-secondary/50 text-sm">
                            ...
                            <p style={bodyFont}>
                              {msg('allowLocationAccess', locale)}
                            </p>
                          </div>
                        )}
                      </div>
...
function NextSignificantCard({
  ...
  const subtypeLabel = {
    total: msg('typeTotal', locale),
    annular: msg('typeAnnular', locale),
    partial: msg('typePartial', locale),
    hybrid: msg('typeHybrid', locale),
    penumbral: msg('penumbralLabel', locale),
  ...
  return (
    ...
    >
      <p className="text-text-secondary/50 text-xs uppercase tracking-wider font-bold mb-3">
        {msg('nextSignificantEclipse', locale)}
      </p>
      <button
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              <span className="text-xs font-mono text-gold-light/70">
                {msg('mag', locale)} {next.magnitude.toFixed(2)}
              </span>
            </div>
          ...
          <div className="shrink-0 flex flex-col items-center">
            ...
            <span className="text-[9px] text-gold-primary/40 mt-1">{msg('viewDetails', locale)}</span>
          </div>
        </div>
      </button>
    </motion.div>
  ...
  return (
    <div className={`rounded-xl bg-gradient-to-br from-[#2d1b69]/30 via-[#1a1040]/35 to-[#0a0e27] border ${borderColor} p-5`}>
      ...
      <div className="space-y-3 text-xs leading-relaxed" style={bodyFont}>
        <div>
          <span className="text-gold-dark text-[10px] uppercase tracking-wider font-bold">{msg('mundaneEffects', locale)}</span>
          <p className="text-text-secondary/80 mt-0.5">{tl(imp.mundane, locale)}</p>
        </div>
        <div>
          <span className="text-gold-dark text-[10px] uppercase tracking-wider font-bold">{msg('personalEffects', locale)}</span>
        ...
        <div>
          <span className="text-gold-dark text-[10px] uppercase tracking-wider font-bold">{msg('recommendedRemedies', locale)}</span>
...
function getMagnitudeLabel(mag: number, isSolar: boolean, locale: string): string {
  if (isSolar) {
    ...
    if (mag >= 1.0) return msg('solarTotalDesc', locale);
  ...
  if (mag >= 1.5) return msg('lunarDeepTotal', locale);
  if (mag >= 1.0) return msg('lunarTotal', locale);
  ...
  return msg('penumbralOnlyDesc', locale);
}
