...
const result = scoreSurgery(kundali, strength, signatures, 'en');

describe('scoreSurgery', () => {
  ...
  it('returns correct metadata matching ELEMENT_CATALOG[surgery]', () => {
    const meta = ELEMENT_CATALOG['surgery'];
    expect(result.id).toBe('surgery');
