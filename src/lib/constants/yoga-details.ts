...
// required (the canonical yoga store always ships hi) and add `mai?`
// for the option A pilot overlay.
import type { LocaleText as GlobalLocaleText } from '@/types/panchang';
interface LocaleText extends GlobalLocaleText {
  hi: string;
  sa?: string;
  // Option A pilot — mai injected at runtime by yoga-details-with-overlay.ts
  // from yoga-mai-overlay.json (generated by /tmp/translate-yoga-mai-gemini.ts).
  // Spec docs/specs/2026-06-04-noindex-thin-translation-locales.md §3 (state 2:
  // partially translated — PER_ROUTE_INDEXABLE flips per slug).
  mai?: string;
}
...
export interface YogaDetailEntry {
  ...
  // Option A pilot — `mai` injected at runtime via yoga-mai-overlay.json.
  detailedDescription: { en: string[]; hi: string[]; mai?: string[] };
  effects: YogaEffect[];
