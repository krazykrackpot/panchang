#!/usr/bin/env python3
"""
Translate CITY_FESTIVAL_CONTEXT (150 festival×city paragraphs) to 7 visible
regional locales via Gemini 2.5 Flash on Vertex AI.

Output: src/lib/constants/city-festival-context-{locale}-overlay.json, keyed by
"<festivalSlug>:<citySlug>".
...
JOBS_FILE = Path("/tmp/cfc-jobs.json")
...
def _write_overlay(locale: str, overlay: dict[str, str]) -> None:
    out_path = OUT_DIR / f"city-festival-context-{locale}-overlay.json"
    merged: dict[str, str] = {}
    if out_path.exists():
        try:
