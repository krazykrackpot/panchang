...
const result = scoreImmunity(kundali, strength, signatures, 'en');

describe('scoreImmunity', () => {
  ...
  it('returns correct metadata matching ELEMENT_CATALOG[immunity]', () => {
    const meta = ELEMENT_CATALOG['immunity'];
    expect(result.id).toBe('immunity');
