...
export default function NakshatraDetailPage() {
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/panchang/nakshatra" className="text-gold-primary hover:text-gold-light mt-4 inline-block">
          &larr; {(locale !== 'hi' && String(locale) !== 'sa') ? 'Back to Nakshatras' : 'नक्षत्रों पर वापस'}
        </Link>
      </div>
    );
  }
  ...
  const sections = [
    { title: (locale !== 'hi' && String(locale) !== 'sa') ? 'Meaning & Etymology' : 'अर्थ और व्युत्पत्ति', content: detail.meaning[locale], color: 'gold-primary' },
    { title: (locale !== 'hi' && String(locale) !== 'sa') ? 'Mythology & Legend' : 'पौराणिक कथा', content: detail.mythology[locale], color: 'gold-light' },
    { title: (locale !== 'hi' && String(locale) !== 'sa') ? 'Significance' : 'महत्व', content: detail.significance[locale], color: 'emerald-400' },
    { title: (locale !== 'hi' && String(locale) !== 'sa') ? 'Personality & Characteristics' : 'व्यक्तित्व और विशेषताएं', content: detail.characteristics[locale], color: 'gold-primary' },
    { title: (locale !== 'hi' && String(locale) !== 'sa') ? 'Favorable Activities' : 'अनुकूल गतिविधियां', content: detail.compatibleActivities[locale], color: 'emerald-400' },
    { title: (locale !== 'hi' && String(locale) !== 'sa') ? 'Remedies & Worship' : 'उपाय और पूजा', content: detail.remedies[locale], color: 'indigo-400' },
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <Link href="/panchang/nakshatra" className="inline-flex items-center gap-2 text-gold-primary hover:text-gold-light mb-8 transition-colors">
        <ArrowLeft className="w-4 h-4" /> {(locale !== 'hi' && String(locale) !== 'sa') ? 'All Nakshatras' : 'सभी नक्षत्र'}
      </Link>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
        {[
          { label: (locale !== 'hi' && String(locale) !== 'sa') ? 'Deity' : 'देवता', value: nak.deity[locale] },
          { label: (locale !== 'hi' && String(locale) !== 'sa') ? 'Ruler' : 'स्वामी', value: nak.rulerName[locale] },
          { label: (locale !== 'hi' && String(locale) !== 'sa') ? 'Nature' : 'स्वभाव', value: nak.nature[locale] },
          { label: (locale !== 'hi' && String(locale) !== 'sa') ? 'Gana' : 'गण', value: detail.gana[locale] },
          { label: (locale !== 'hi' && String(locale) !== 'sa') ? 'Guna' : 'गुण', value: detail.guna[locale] },
          { label: (locale !== 'hi' && String(locale) !== 'sa') ? 'Tattva' : 'तत्व', value: detail.tattva[locale] },
          { label: (locale !== 'hi' && String(locale) !== 'sa') ? 'Animal' : 'पशु', value: detail.associatedAnimal[locale] },
          { label: (locale !== 'hi' && String(locale) !== 'sa') ? 'Degrees' : 'अंश', value: `${nak.startDeg.toFixed(1)}° — ${nak.endDeg.toFixed(1)}°` },
        ].map((item, i) => (
          <motion.div
            key={item.label}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-amber-300 font-semibold text-sm mb-1" style={headingFont}>
                {(locale !== 'hi' && String(locale) !== 'sa') ? 'Ganda Mula Nakshatra' : 'गण्ड मूल नक्षत्र'}
              </h4>
              ...
              >
                {(locale !== 'hi' && String(locale) !== 'sa') ? 'Learn about Ganda Mula Nakshatras & Remedies →' : 'गण्ड मूल नक्षत्र और उपाय जानें →'}
              </Link>
            </div>
          </div>
        </motion.div>
      ...
      <section className="my-10">
        <h2 className="text-2xl font-bold text-gold-gradient mb-6" style={headingFont}>
          {(locale !== 'hi' && String(locale) !== 'sa') ? 'Four Padas (Quarters)' : 'चार पाद'}
        </h2>
            ...
            return (
              ...
              >
                ...
                <div className="text-gold-dark text-xs uppercase tracking-wider font-bold mb-2">
                  {(locale !== 'hi' && String(locale) !== 'sa') ? `Pada ${pada}` : `पाद ${pada}`}
                </div>
                ...
                <div className="text-gold-light text-sm font-semibold" style={bodyFont}>
                  {(locale !== 'hi' && String(locale) !== 'sa') ? `Navamsha: ${rashiNames[navamshaRashi - 1]}` : `नवांश: ${rashiNamesHi[navamshaRashi - 1]}`}
                </div>
              </motion.div>
            );
          })}
      ...
      <div className="flex justify-between items-center my-10">
        ...
        >
          ...
          <div>
            <div className="text-gold-dark text-xs">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Previous' : 'पिछला'}</div>
        ...
        >
          <div className="text-right">
            <div className="text-gold-dark text-xs">{(locale !== 'hi' && String(locale) !== 'sa') ? 'Next' : 'अगला'}</div>
