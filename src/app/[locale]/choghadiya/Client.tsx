...
export default function ChoghadiyaClient({ urlDate }: ChoghadiyaClientProps = {}) {
  ...
  // (React #418). When on the dynamic /choghadiya route, compute today
  // in the selected city's timezone (prevents across-midnight day-flip).
  ...
  const dateStr = useMemo(() => {
    ...
    if (urlDate?.dateStr) {
      // For the dated route: parse the URL date string with Date.UTC to avoid
      // local-TZ interpretation (Lesson L). Use timeZone:'UTC' in formatting
      // for the same reason — the date itself is the canonical data, not "today".
    ...
    // Dynamic /choghadiya route: local-time constructor is fine here because
