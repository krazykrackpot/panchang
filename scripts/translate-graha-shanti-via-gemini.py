#!/usr/bin/env python3
"""
Translate GRAHA_SHANTI (9 planets × ~10 LocaleText fields) to 7 visible
regional locales via Gemini 2.5 Flash on Vertex AI.

Output: src/lib/constants/graha-shanti-{locale}-overlay.json, keyed by
"<planetId>.<field>" / "<planetId>.mantra.meaning".
...
JOBS_FILE = Path("/tmp/gs-jobs.json")
...
def _write_overlay(locale: str, overlay: dict[str, str]) -> None:
    out_path = OUT_DIR / f"graha-shanti-{locale}-overlay.json"
    merged: dict[str, str] = {}
    if out_path.exists():
        try:
