...
import M from '@/messages/pages/kundali-detail.json';
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center">
        ...
        <p className="text-text-secondary">{msg('loadingShared')}</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-12">
        <Link href="/kundali" className="inline-flex items-center gap-2 text-gold-primary hover:text-gold-light mb-8">
          <ArrowLeft className="w-4 h-4" /> {msg('generateNew')}
        </Link>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleCopy} className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-lg text-sm text-gold-primary hover:text-gold-light">
            ...
            {copied ? msg('copied') : msg('copyLink')}
          </button>
          {typeof navigator !== 'undefined' && 'share' in navigator && (
            <button onClick={() => navigator.share({ title: `${chartData.name} - Kundali`, url: shareUrl })}
              ...
              <Share2 className="w-4 h-4" /> {msg('share')}
            </button>
          )}
        </div>
      </div>
      ...
      {kundali && (
        <div className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-8 mb-8">
          <h2 className="text-xl text-gold-gradient font-bold mb-6" style={headingFont}>
            {msg('planetaryPositions')}
          </h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-gold-primary border-b border-gold-primary/10">
                  <th className="text-left py-2 px-3">{msg('planet')}</th>
                  <th className="text-left py-2 px-3">{msg('sign')}</th>
                  <th className="text-right py-2 px-3">{msg('degree')}</th>
                  <th className="text-center py-2 px-3">{msg('house')}</th>
                  <th className="text-center py-2 px-3">{msg('nakshatra')}</th>
                </tr>
              </thead>
              <tbody>
        ...
        return (
          <div className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-xl p-8 mb-8">
            ...
            <div className="space-y-3">
              {afflicted.map((ap) => {
                const cfg = {
                  severe: { border: 'border-rose-500/20', bg: 'bg-rose-500/8', text: 'text-rose-400', badge: 'bg-rose-500/20 text-rose-400', label: msg('severe') },
                  moderate: { border: 'border-amber-500/20', bg: 'bg-amber-500/8', text: 'text-amber-400', badge: 'bg-amber-500/20 text-amber-400', label: msg('moderate') },
                  mild: { border: 'border-blue-500/20', bg: 'bg-blue-500/8', text: 'text-blue-400', badge: 'bg-blue-500/20 text-blue-400', label: msg('mild') },
                }[ap.severity];
