...
const LABELS: Record<string, Record<string, string>> = {
  ...
  transitVedha: {
    en: "Transit Vedha Analysis",
    hi: "गोचर वेध विश्लेषण",
    sa: "गोचरवेधविश्लेषणम्",
  },
  ...
  yourNakshatra: {
    en: "YOUR nakshatra is under vedha",
    hi: "आपका नक्षत्र वेध में है",
    sa: "भवतः नक्षत्रं वेधे अस्ति",
  },
  ...
  overallAssessment: {
    ...
    hi: "समग्र मूल्यांकन",
    sa: "समग्रमूल्याङ्कनम्",
  },
  noTransits: {
    en: "Select a birth nakshatra to begin analysis",
    hi: "विश्लेषण के लिए जन्म नक्षत्र चुनें",
    sa: "विश्लेषणार्थं जन्मनक्षत्रं चिनुत",
  },
  ...
  hoverHint: {
    en: "Hover/tap a cell for details",
    hi: "विवरण के लिए सेल पर क्लिक करें",
    sa: "विवरणार्थं कोष्ठे क्लिक् कुर्वन्तु",
  },
  noneThisPeriod: {
    en: "None in this period",
    hi: "इस अवधि में कोई नहीं",
    sa: "अस्मिन् काले न किमपि",
  },
};
...
export default function SarvatobhadraClient() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        ...
        const moonNak = kundali?.planets?.find(
          (p: { planet: { id: number } }) => p.planet.id === 1,
        )?.nakshatra?.id;
        if (moonNak && moonNak >= 1 && moonNak <= 27) setBirthNakId(moonNak);
      }
    } catch {
      /* sessionStorage unavailable or parse error  –  user picks manually */
    }
  }, [searchParams]);
  ...
  const transitInput = useMemo(
    () =>
      currentTransits.map((t) => ({
        planetId: t.planetId,
        nakshatraId: t.nakshatraId,
      })),
    [currentTransits],
  );
    ...
    for (const tv of analysis.transitVedhas) {
      ...
      for (const row of SBC_GRID) {
        for (const cell of row) {
          if (
            cell.type === "nakshatra" &&
            cell.nakshatraId === tv.nakshatraId
          ) {
  ...
  const birthIsStruck = useCallback(
    (tv: TransitVedha): boolean => {
      return tv.vedhaLines.some((l) =>
        l.affectedNakshatras.includes(birthNakId),
      );
    },
    [birthNakId],
  );
  ...
  const getCellClasses = useCallback(
    (cell: SBCCell): string => {
      ...
      const base =
        "flex items-center justify-center border text-center transition-all duration-150 cursor-pointer select-none";
      const sizeClass =
        "w-full min-h-[44px] sm:min-h-[52px] md:min-h-[56px] text-[10px] sm:text-xs md:text-sm";
  ...
  const getCellLabel = useCallback(
    (cell: SBCCell): string => {
      if (cell.type === "nakshatra") {
        ...
        if (locale === "en")
          return fullName.length > 8 ? fullName.slice(0, 7) : fullName;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
      {/* Header (h1 + subtitle) is server-rendered in page.tsx so Bingbot
          sees the heading in the initial HTML. See page.tsx comment. */}

      {/* Educational context */}
      <details
        className="mb-8 bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-2xl group"
        open
      >
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 sm:grid-cols-4">
                {LEGEND_ITEMS.map((item) => (
                  <div key={item.key} className="flex items-center gap-1.5">
                    <div
                      className={`h-3 w-3 rounded-sm border ${item.bg} ${item.border}`}
                    />
              ...
              <div className="bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] border border-gold-primary/12 rounded-2xl p-4 sm:p-6">
                ...
                {currentTransits.length === 0 ? (
                  ...
                  (() => {
                    ...
                    const allExits = currentTransits.map((t) =>
                      t.exitDate.getTime(),
                    );
                    const allEntries = currentTransits.map((t) =>
                      t.entryDate.getTime(),
                    );
                    const timelineStart = new Date(
                      Math.min(now.getTime() - 30 * 86400000, ...allEntries),
                    );
                    const timelineEnd = new Date(
                      Math.max(now.getTime() + 60 * 86400000, ...allExits),
                    );
                    const totalMs =
                    ...
                    const toPercent = (d: Date) =>
                      Math.max(
                        ...
                        Math.min(
                          100,
                          ((d.getTime() - timelineStart.getTime()) / totalMs) *
                            100,
                        ),
                      );
                    ...
                    const sorted = [...currentTransits].sort(
                      (a, b) =>
                        b.exitDate.getTime() -
                        b.entryDate.getTime() -
                        (a.exitDate.getTime() - a.entryDate.getTime()),
                    );
                    ...
                    while (mCursor < timelineEnd) {
                      months.push({
                        label: mCursor.toLocaleDateString("en", {
                          month: "short",
                          year: "2-digit",
                        }),
                      ...
                      <div className="relative">
                        ...
                        <div className="relative h-5 mb-1">
                          {months.map((m, i) => (
                            <span
                              key={i}
                              className="absolute text-[9px] text-text-secondary/40 -translate-x-1/2"
                              style={{ left: `${m.pct}%` }}
                            >
                              {m.label}
                            </span>
                        ...
                        <div className="space-y-1.5">
                          {sorted.map((t) => {
                            const tv = analysis.transitVedhas.find(
                              (v) => v.planetId === t.planetId,
                            );
                            ...
                            return (
                              ...
                              >
                                <div className="w-14 sm:w-18 shrink-0 text-right">
                                  ...
                                  >
                                    {graha?.symbol}{" "}
                                    {tl(tv?.planetName ?? { en: "" }, locale)}
                                  </span>
                                </div>
                                <div className="flex-1 min-w-0 relative h-5">
                                  {/* Today marker */}
                                  <div
                                    className="absolute top-0 bottom-0 w-px bg-gold-primary/30"
                                    style={{ left: `${todayPct}%` }}
                                  />
                                  {/* Bar */}
                                  <div
                                    className={`absolute top-0 h-full rounded-sm flex items-center justify-center overflow-hidden ${
                                      isBenefic
                                        ? struck
                                          ? "bg-emerald-500/50 border border-emerald-400/60"
                                          : "bg-emerald-500/15 border border-emerald-400/15"
                                        : struck
                                          ? "bg-red-500/50 border border-red-400/60"
                                          : "bg-red-500/15 border border-red-400/15"
                                    }`}
                                    style={{
                                      left: `${leftPct}%`,
                                      width: `${widthPct}%`,
                                  ...
                                  >
                                    {widthPct > 8 && (
                                      <span
                                        className={`text-[8px] sm:text-[9px] font-medium truncate px-1 ${
                                          isBenefic
                                            ? "text-emerald-300"
                                            : "text-red-300"
                                ...
                                {struck && (
                                  <span
                                    className={`text-[9px] shrink-0 ${isBenefic ? "text-emerald-400" : "text-red-400"}`}
                                  >
                                    ●
                                  </span>
                                )}
        ...
        <div className="mt-8 space-y-6">
          <div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {analysis.transitVedhas.map((tv) => {
                ...
                const transit = currentTransits.find(
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2">
                        <span
                          className="text-lg"
                          style={{
                            color: GRAHAS.find((g) => g.id === tv.planetId)
                              ?.color,
                          }}
                        >
                    ...
                    {transit && (
                      <p className="text-[11px] text-text-secondary/70 mb-1">
                        {formatD(transit.entryDate)} –{" "}
                        {formatD(transit.exitDate)}
                    ...
                    {isStruck && (
                      <div
                        className={`mt-1 mb-1 flex items-center gap-1.5 rounded px-2 py-0.5 text-[11px] font-medium ${
                          tv.isBenefic
          ...
          <div>
            ...
            <div className="mb-4 rounded-xl border border-gold-primary/15 bg-gradient-to-br from-[#2d1b69]/40 via-[#1a1040]/50 to-[#0a0e27] p-4">
              <h3 className="mb-1 text-xs font-semibold text-gold-light uppercase tracking-wider">
                {l("overallAssessment")}
              </h3>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-emerald-400/15 bg-emerald-500/5 p-4">
                <h3 className="mb-2 text-xs font-semibold text-emerald-400 uppercase tracking-wider">
                  {l("favorableDays")}
                </h3>
                {analysis.favorableDays.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5">
                    {analysis.favorableDays.map((d) => (
                      <span
                        key={d}
                        className="rounded bg-emerald-500/10 px-2.5 py-1 text-xs text-emerald-300"
                      >
                        {tl(WEEKDAY_NAMES[d], locale)}
                      </span>
                    ))}
                  </div>
                ) : (
                ...
                {analysis.unfavorableDays.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5">
                    {analysis.unfavorableDays.map((d) => (
                      <span
                        key={d}
                        className="rounded bg-red-500/10 px-2.5 py-1 text-xs text-red-300"
                      >
                        {tl(WEEKDAY_NAMES[d], locale)}
                      </span>
                    ))}
