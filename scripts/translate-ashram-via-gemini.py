#!/usr/bin/env python3
"""
Translate ASHRAMS (4 life stages × description + focusAreas) to 7 visible
regional locales via Gemini 2.5 Flash on Vertex AI.

Output: src/lib/constants/ashram-data-{locale}-overlay.json, keyed by
"<id>.description" / "<id>.focusAreas[N]".
...
JOBS_FILE = Path("/tmp/ashram-jobs.json")
...
def _write_overlay(locale: str, overlay: dict[str, str]) -> None:
    out_path = OUT_DIR / f"ashram-data-{locale}-overlay.json"
    merged: dict[str, str] = {}
    if out_path.exists():
        try:
