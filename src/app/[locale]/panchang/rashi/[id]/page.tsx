...
const LABELS = {
  ...
  personality: {
    ...
    hi: "व्यक्तित्व एवं लक्षण",
    sa: "व्यक्तित्वं लक्षणानि च",
  },
  strengths: { en: "Strengths", hi: "शक्तियाँ", sa: "शक्तयः" },
  challenges: { en: "Challenges", hi: "चुनौतियाँ", sa: "आह्वानानि" },
  career: {
    en: "Career & Profession",
    hi: "करियर एवं व्यवसाय",
    sa: "वृत्तिः व्यवसायश्च",
  },
  health: {
    en: "Health & Wellness",
    hi: "स्वास्थ्य एवं कल्याण",
    sa: "स्वास्थ्यं कल्याणं च",
  },
  relationships: {
    en: "Relationships & Love",
    hi: "रिश्ते एवं प्रेम",
    sa: "सम्बन्धाः प्रेमश्च",
  },
  ...
  loading: {
    en: "Computing horoscope...",
    hi: "राशिफल गणना हो रही है...",
    sa: "राशिफलगणना प्रचलति...",
  },
  compatible: {
    en: "Compatible Signs",
    hi: "अनुकूल राशियाँ",
  ...
  relatedNakshatras: {
    en: "Nakshatras in this Rashi",
    hi: "इस राशि के नक्षत्र",
    sa: "अस्यां राश्यां नक्षत्राणि",
  },
  faq: {
    en: "Frequently Asked Questions",
    hi: "अक्सर पूछे जाने वाले प्रश्न",
    sa: "बहुधा पृच्छ्यमानाः प्रश्नाः",
  },
...
function ScoreBar({ score, max = 10 }: { score: number; max?: number }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-1">
      ...
      <span className="text-xs text-text-secondary font-mono w-6 text-right">
        {score}/{max}
      </span>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        {open ? (
          <ChevronUp className="w-4 h-4 text-gold-dark flex-shrink-0" />
        ) : (
          <ChevronDown className="w-4 h-4 text-gold-dark flex-shrink-0" />
        )}
      </button>
      <AnimatePresence>
        {open && (
...
export default function RashiDetailPage() {
  ...
  const detail = rashi
    ? RASHI_DETAILS.find((d) => d.id === rashi.id)
    : undefined;
  ...
  useEffect(() => {
    if (!rashi) return;
    import("@/lib/horoscope/daily-engine").then(
      ({ generateDailyHoroscope }) => {
        const today = new Date();
        const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, "0")}-${String(today.getDate()).padStart(2, "0")}`;
        setHoroscope(
          generateDailyHoroscope({ moonSign: rashi.id, date: dateStr }),
        );
      },
    );
  }, [rashi]);
  ...
  const elemBorder =
    ELEMENT_BORDER[rashi.element.en] || "border-gold-primary/20";
  ...
  // Compatible rashis
  const compatRashis = detail.compatibleRashis
    .map((id) => RASHIS[id - 1])
    .filter(Boolean);

  // Breadcrumb JSON-LD
  const breadcrumbLD = generateBreadcrumbLD(
    `/${locale}/panchang/rashi/${slug}`,
    locale,
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-8">
          ...
          <div className="text-center sm:text-left flex-1">
            <div className="text-gold-dark text-sm font-mono mb-2">
              #{rashi.id} {tl(LABELS.of12, locale)} &middot; {rashi.startDeg}
              &deg; &mdash; {rashi.endDeg}&deg;
            </div>
            <h1
            ...
            {locale === "en" && (
              <p
                className="text-gold-dark text-lg mb-2"
            ...
            <div className="flex flex-wrap justify-center sm:justify-start gap-2 mt-3">
              <span
                className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${elemBg} ${elemText} border ${elemBorder}`}
              >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-10">
        {[
          {
            icon: <Star className="w-4 h-4 text-gold-primary" />,
            label: tl(LABELS.ruler, locale),
            value: rashi.rulerName[locale] || rashi.rulerName.en,
          },
          {
            icon: <Flame className="w-4 h-4" style={{ color: elemColor }} />,
            label: tl(LABELS.element, locale),
            value: rashi.element[locale] || rashi.element.en,
          },
          {
            icon: <Sparkles className="w-4 h-4 text-purple-400" />,
            label: tl(LABELS.quality, locale),
            value: rashi.quality[locale] || rashi.quality.en,
          },
          {
            icon: <Hash className="w-4 h-4 text-gold-dark" />,
            label: tl(LABELS.degrees, locale),
            value: `${rashi.startDeg}\u00B0 \u2013 ${rashi.endDeg}\u00B0`,
          },
          {
            icon: <Hash className="w-4 h-4 text-emerald-400" />,
            label: tl(LABELS.luckyNumbers, locale),
            value: detail.luckyNumbers.join(", "),
          },
          {
            icon: <Palette className="w-4 h-4 text-pink-400" />,
            label: tl(LABELS.luckyColors, locale),
            value: tl(detail.luckyColors, locale),
          },
          {
            icon: <Gem className="w-4 h-4 text-cyan-400" />,
            label: tl(LABELS.luckyGems, locale),
            value: tl(detail.luckyGems, locale),
          },
          {
            icon: <Star className="w-4 h-4 text-amber-400" />,
            label: tl(LABELS.of12, locale),
            value: `#${rashi.id}`,
          },
        ].map((item, idx) => (
          <motion.div
          ...
          >
            <div className="flex items-center gap-2 mb-1.5">
              {item.icon}
              <span className="text-gold-dark text-xs" style={bodyFont}>
                {item.label}
              </span>
            </div>
            <div
      ...
      <section className="my-12">
        <h2
          className="text-2xl font-bold text-gold-gradient mb-6"
          style={headingFont}
        >
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-lg p-4">
              <h3
                className="text-emerald-400 font-semibold text-sm mb-2"
                style={headingFont}
              >
            ...
            <div className="bg-amber-500/5 border border-amber-500/15 rounded-lg p-4">
              <h3
                className="text-amber-400 font-semibold text-sm mb-2"
                style={headingFont}
              >
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="flex-1">
                <div className="text-gold-dark text-xs mb-1">
                  {tl(LABELS.overallScore, locale)}
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
              {(
                Object.keys(horoscope.areas) as Array<
                  keyof typeof horoscope.areas
                >
              ).map((area) => {
                  ...
                  <div key={area} className="bg-bg-primary/40 rounded-lg p-3">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className="text-gold-light text-xs font-semibold">
                        {tl(AREA_LABELS[area], locale)}
                      </span>
                    </div>
                    <ScoreBar score={areaData.score} />
                    <p
                      className="text-text-secondary text-xs mt-1.5 leading-relaxed"
                      style={bodyFont}
                    >
            ...
            <div className="bg-bg-primary/40 rounded-lg p-4 mb-4">
              <h4 className="text-gold-light text-sm font-semibold mb-2">
                {tl(LABELS.insight, locale)}
              </h4>
              <p
                className="text-text-secondary text-sm leading-relaxed"
                style={bodyFont}
              >
                {horoscope.insight[locale as "en" | "hi"] ||
                  horoscope.insight.en}
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-bg-primary/40 rounded-lg p-3 text-center">
                <div className="text-gold-dark text-xs mb-1">
                  {tl(LABELS.luckyColor, locale)}
                </div>
                <div className="text-gold-light text-sm font-semibold">
                  {horoscope.luckyColor[locale as "en" | "hi"] ||
                    horoscope.luckyColor.en}
                </div>
              </div>
              <div className="bg-bg-primary/40 rounded-lg p-3 text-center">
                <div className="text-gold-dark text-xs mb-1">
                  {tl(LABELS.luckyNumber, locale)}
                </div>
                <div className="text-gold-light text-sm font-semibold font-mono">
                  {horoscope.luckyNumber}
                </div>
              </div>
              <div className="bg-bg-primary/40 rounded-lg p-3 text-center">
                <div className="text-gold-dark text-xs mb-1">
                  {tl(LABELS.luckyTime, locale)}
                </div>
                <div className="text-gold-light text-sm font-semibold font-mono">
                  {horoscope.luckyTime}
                </div>
            ...
            return (
              <Link key={cr.id} href={`/matching/${s1}-and-${s2}`}>
                ...
                >
                  ...
                  <div
                    className={`text-xs mt-1 ${ELEMENT_TEXT[cr.element.en] || "text-text-secondary"}`}
                  >
      ...
      <section className="my-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {relatedNakshatras.map((nak, idx) => {
            ...
            const overlapPct = (
              ((overlapEnd - overlapStart) / (nak.endDeg - nak.startDeg)) *
              100
            ).toFixed(0);
            return (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-text-secondary text-xs">
                        {nak.startDeg.toFixed(1)}&deg; &ndash;{" "}
                        {nak.endDeg.toFixed(1)}&deg;
                        <span className="text-gold-dark ml-2">
                          ({overlapPct}% in {rashi.name.en})
                        </span>
                      </div>
                    </div>
      ...
      <div className="flex items-center justify-between my-12">
        ...
        >
          ...
          <div className="text-right">
            <div className="text-gold-dark text-xs">
              {tl(LABELS.previous, locale)}
            </div>
