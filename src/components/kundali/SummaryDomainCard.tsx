  ...
  return (
    ...
    >
      ...
      <div className="p-5 sm:p-6">
        {/* ---- Header: rings + domain name + tier label ---- */}
        <div className="mb-4 flex items-start gap-4">
          <DomainRings
            ...
            ariaLabel={isDevanagariLocale(locale)
              ? `${domainName}: ${tl(domain.overallRating.label, locale)}, जीवन चरण ${Math.round(domain.currentActivation.dashaActivationScore * 10)}%, अभी ${Math.round(domain.currentActivation.overallActivationScore * 10)}%`
              : `${domainName}: ${tl(domain.overallRating.label, locale)}, Life Phase ${Math.round(domain.currentActivation.dashaActivationScore * 10)}%, Right Now ${Math.round(domain.currentActivation.overallActivationScore * 10)}%`}
          />
